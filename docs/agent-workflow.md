...
source ~/.nvm/nvm.sh
npm run build
npm run start -- --hostname 0.0.0.0 --port 9002
```
...
cmd.exe /V:ON /C "cd /d F:\dev\python-kraddr-geo-codex\kraddr-geo-ui && set PLAYWRIGHT_BASE_URL=http://<WSL_IP>:9002&& npx playwright test --config playwright.config.ts --project chromium --workers 1"
cmd.exe /V:ON /C "cd /d F:\dev\python-kraddr-geo-codex\kraddr-geo-ui && set PLAYWRIGHT_BASE_URL=http://<WSL_IP>:9002&& npx playwright test --config playwright.config.ts --project firefox --workers 1"
```
...
```bash
source ~/.nvm/nvm.sh
node -e "fetch('http://127.0.0.1:9002/api/runtime-config').then(r=>r.json()).then(j=>console.log('vworld_key_nonempty=' + Boolean(j.vworldApiKey) + ', length=' + String(j.vworldApiKey || '').length))"
```
...
| FastAPI 백엔드 | `9001` | `uvicorn kraddr.geo.api.app:app --host 127.0.0.1 --port 9001` |
| `kraddr-geo-ui` | `9002` | `npm run dev -- --port 9002`, Playwright base URL도 9002 |
| RustFS S3 API | `9003` | Docker 내부 endpoint `http://kraddr-geo-rustfs:9003` |
| RustFS console | `9004` | 로컬 운영자 콘솔 |

기본 `5432`/`3000`은 다른 프로젝트와 충돌하므로 외부 진입점으로 쓰지 않는다. RustFS의 upstream 예시 기본 포트 `9000`/`9001`도 이 저장소에서는 API/UI와 충돌하므로 쓰지 않는다.
