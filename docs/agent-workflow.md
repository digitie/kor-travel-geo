...
```bash
source ~/.nvm/nvm.sh
node -e "fetch('http://127.0.0.1:13088/api/runtime-config').then(r=>r.json()).then(j=>console.log('vworld_key_nonempty=' + Boolean(j.vworldApiKey) + ', length=' + String(j.vworldApiKey || '').length))"
```
