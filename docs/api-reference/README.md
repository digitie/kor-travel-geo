...
- v2는 데이터셋 변경 감지 표면도 제공한다: `POST /v2/dataset/version`(폴링 대상, 현재
  버전+변경 감지), `POST /v2/dataset/history`(변경 이력). candidate 표면과 달리 Python
  라이브러리로는 공개하지 않는다(REST 전용, ADR-039). 상세는
  [v2 dataset version](v2/dataset-version.md), 결정은 [ADR-067](../adr/067-external-dataset-version-api.md).
...
- [v2 dataset version](v2/dataset-version.md)
