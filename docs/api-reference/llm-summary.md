...
- v2는 candidate-list 표면이다. `candidates[]`의 `address`, `point`, `bbox`, `region`, `place`, `source`, `metadata`를 보고 자체 로컬 데이터를 풍부하게 해석한다.
...
- 기존 vworld 호환 소비자, 운영 UI, 과거 스크립트는 v1을 사용한다.
- 신규 애플리케이션, provider 비교, C1~C10 분석 UI처럼 후보별 출처와 metadata가 중요한 화면은 v2를 사용한다.
