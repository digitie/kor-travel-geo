...
| `type` | `both`, `road`, `parcel` (소문자) | `both` | 반환 주소 유형 |
...
> ⚠️ 입력 `type`은 **소문자**(`both`/`road`/`parcel`)만 받는다. 응답은 대문자로 직렬화되므로 응답값을 그대로 되넣으면(`type=ROAD`) `400 INVALID_TYPE`가 난다(geocode와 동일). 대소문자 무관 허용은 후속 과제.
