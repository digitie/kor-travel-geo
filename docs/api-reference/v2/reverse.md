...
`candidates[]`는 도로명/지번 후보를 함께 담는다. 각 후보는 `match_kind`, `address`, `point`, `region`, `source`, `distance_m`, `confidence`를 포함할 수 있다.
...
```bash
curl -X POST "http://localhost:8888/v2/reverse" \
