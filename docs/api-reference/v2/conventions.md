...
- **반영 ✅ (T-266/#308)**: `ReverseV2Input`/`SearchV2Input`에 `include_geometry`(기본 false)를 추가하고, geocode의 후보 도형 enrich 로직을 공용 `_enrich_candidates_with_geometry`로 추출해 세 endpoint가 동일 규칙(`region`→region polygon, `road`/`parcel`+건물 key→building polygon, else `null`)을 쓴다. search `type="district"` 후보가 region 도형을 받는다. reverse local 후보는 건물 key가 없어 현재 `null`이며(점 기반 건물 조회는 후속), geocode 동작은 무변경이다. `bbox`(reverse 제외)는 현행 유지.
...
1. **additive·doc 먼저** — §5 reverse/search `include_geometry` 추가 **✅(T-266/#308)**, §3·§6·§7 문서화·§1 envelope 목표 명문화는 후속(T-267). (non-breaking)
