...
- **현재**: v2 검증/도메인 에러는 공유 `{response:{status:"ERROR", errorCode, errorMessage, hint?}}`를 쓴다(`api/responses.py`). ADR-061(#316) 이후 `/v2/geocode`·`/v2/reverse`·`/v2/search`는 `responses={400: StructuredErrorEnvelope}`로 400을 명세하고 OpenAPI customization으로 자동 422를 제거한다. 다만 (a) `regions/within-radius` 등 비-address 경로는 여전히 자동 422만 광고하고(런타임은 400), (b) 성공과 같은 trace 키(`query_id`)를 담는 v2 전용 error 모델은 아직 없다.
