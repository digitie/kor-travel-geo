...
DBF deleted record는 iterator에서 건너뛰며, coverage 비교용 `row_count`도 header의 원시 record 수가 아니라 deleted flag가 없는 실제 검증 대상 record 수로 계산한다.
...
- `row_count`: DBF deleted record를 제외한 검증 대상 행 수
...
`row_limit_per_layer`가 있으면 `checked_count`와 mismatch count는 sample 기준이다. 단, 도형 ZIP coverage용 `row_count`는 별도 DBF stream scan으로 산출하므로 제한 실행에서도 전체 non-deleted row 수를 유지한다.
