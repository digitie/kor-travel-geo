...
- `python-kraddr-geo`, `python-krtour-map`, `tripmate`가 같은 bucket을 쓰더라도 prefix를 분리하는 원칙
- 기존 `data/`를 사용한 live test와 Chrome/Firefox Playwright e2e 검증 절차

## 외부 전제

RustFS는 S3 호환 object storage로 사용한다. 이 저장소는 RustFS를 직접 구동·정지·재시작하지 않고, 이미 동작 중인 endpoint와 bucket 접속 설정만 저장한다. 기본 접속 예시는 `.env.example`의 `KRADDR_GEO_RUSTFS_*` 값을 따른다.
...
서버는 허용된 local import root 아래 파일만 업로드한다. API 컨테이너를 사용할 때는 host `data/`를 읽기 전용으로 제공하고 `KRADDR_GEO_RUSTFS_LOCAL_IMPORT_ROOTS`를 해당 경로에 맞춘다. 사용자가 임의 시스템 경로를 입력해 filesystem 전체를 업로드하지 못하게 한다.
...
1. `.env` 또는 admin UI에 `KRADDR_GEO_RUSTFS_*` 접속 설정을 저장한다.
