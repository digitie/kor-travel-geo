...
- callback retry/backoff와 attempt 기록은 T-050 2차에서 구현했다. 수신자 측 replay 저장소는 이 저장소가 관리하지 않으므로, 운영자가 callback endpoint에서 timestamp window와 callback ID de-duplication을 적용해야 한다.
...
- callback 수신 endpoint 예제와 replay window/de-duplication 운영 가이드.
