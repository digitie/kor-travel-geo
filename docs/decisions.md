...
- (open) T-121 전국 C11~C17 실행에서 본 ADR 임계값을 실제 metric으로 평가한다.
- (open) T-123 최종 검증에서 ADR-051을 accepted로 전환할지, 아니면 C11을 계속 검증 전용으로 둘지 결정한다.
- (open) T-119는 ADR-051 accepted와 사용자 승인 뒤에만 진행한다.
