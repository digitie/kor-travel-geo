...
## ADR-016: 적재 작업 상태는 DB에 영속화하고 실행 락을 DB로 보강한다
...
### 컨텍스트
ADR-006은 `api/_jobs.py`에 `asyncio.Queue`와 `Semaphore(1)` 기반 in-process 큐를 두기로 했다. 이 구조는 초기 구현이 단순하지만, Uvicorn reload, 컨테이너 재기동, 배포 전환, 다중 worker 오동작 시 대기 중이거나 실행 중인 작업 상태가 메모리에서 사라진다. 대량 적재는 master table과 MV를 갱신하므로 작업 상태 소실은 부분 적재와 운영자 판단 오류로 이어질 수 있다.

### 결정
`load_manifest`는 성공한 적재의 watermark만 기록한다. 작업 큐 상태는 별도 `load_jobs` 테이블에 `PENDING`, `RUNNING`, `SUCCESS`, `FAILED`, `CANCELLED` 상태로 영속화한다. 실제 실행 직렬성은 인메모리 semaphore와 함께 PostgreSQL advisory lock 또는 `SELECT ... FOR UPDATE SKIP LOCKED`로 보강한다.

### 근거
- 작업 상태와 적재 결과 watermark는 수명과 의미가 다르다.
- 프로세스 재시작 후에도 운영자가 마지막 작업의 실패·대기·성공 상태를 확인할 수 있어야 한다.
- 단일 인스턴스 가정을 유지하더라도 DB 락을 함께 쓰면 reload와 다중 worker 실수를 방어할 수 있다.

### 결과(긍정)
- `/v1/admin/jobs`가 재시작 이후에도 일관된 작업 목록을 보여줄 수 있다.
- `RUNNING` 작업을 시동 시 `FAILED`로 안전 마크하고, 유효한 `PENDING` 작업은 재큐잉할 수 있다.
- 향후 Redis/RQ 또는 LISTEN/NOTIFY로 확장할 때도 public API를 크게 바꾸지 않는다.

### 결과(부정)
- T-015 구현 범위가 단순 in-memory queue보다 커진다.
- 업로드 파일 보존, checksum 검증, stale heartbeat 판단 기준을 추가로 구현해야 한다.

### 후속
- (open) stale `RUNNING` 판단 기준과 heartbeat 주기를 운영 환경에서 확정.
- (open) DB 락 방식은 T-015 구현 시 advisory lock과 `FOR UPDATE SKIP LOCKED` 중 하나로 선택.
