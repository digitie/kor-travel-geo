...
5. callback URL은 allowlist host만 허용하고, terminal state(`done`, `failed`, `cancelled`)에서 제한된 횟수로 재시도한다. callback 실패는 백업/복원 성공 여부와 별도로 `callback_state`에 기록한다.
...
- 임시 파일은 `.part` 또는 임시 디렉터리에 쓰고, 완료 후 atomic rename한다.
...
- (open) T-046에서 DTO, API router, job handler, CLI, UI를 구현한다.
- (open) `ops.artifacts` 기반 backup metadata persistence를 추가한다. 전용 `db_backup_artifacts`는 신규 구현의 기본 테이블로 만들지 않는다.
- (open) `pg_dump`/`pg_restore` progress parser와 password redaction 테스트를 추가한다.
- (open) 대구광역시 부분 적재 기반 backup → restore 통합 테스트를 추가한다.
...
- (open) T-047에서 benchmark harness, corpus, report schema를 구현한다.
...
## ADR-024: `도로명주소 출입구 정보`는 별도 테이블에 저장하고 MV 대표 출입구 1순위 후보로 사용한다
...
4. `mv_geocode_target` 대표 좌표는 `tl_roadaddr_entrc` → `tl_locsum_entrc` → `tl_navi_buld_centroid` 순서로 선택한다.
...
7. C3/C4/C6/C7/C8은 `tl_roadaddr_entrc`와 `tl_locsum_entrc`를 합친 대표 출입구 CTE를 사용한다.
8. C10은 `tl_roadaddr_entrc.source_yyyymm`을 기준월 비교 대상에 포함한다.
...
- 기준월이 202605 계열이라 기본 full-load에 자동 포함하면 C10 경고가 상시 발생할 수 있다. 따라서 운영자가 명시적으로 적재한 경우에만 MV에 반영한다.
...
- `mv_geocode_target`은 `tl_roadaddr_entrc`가 비어 있으면 기존 locsum/navi 동작과 동일하다.
- `tl_roadaddr_entrc`를 적재한 뒤 `refresh mv`를 실행하면 direct entrance가 대표 좌표로 우선된다.

### 남은 위험

- 전국 full-load에서 `tl_roadaddr_entrc`를 포함했을 때 C3/C4/C6/C7이 얼마나 개선되는지는 T-027 최종 클린 적재에서 다시 측정해야 한다.
- `RNENTDATA_2605_*`와 다른 기준월의 `rnaddrkor_*`, `locsum`, SHP를 섞는 운영 모드에서는 C10 WARN을 정상적인 운영 경고로 해석할지, 또는 기준월을 맞춘 데이터셋만 적재할지 정책이 필요하다.
