...
T-045 source set 업로드 구현은 upload set manifest와 파일 본문을 모두 로컬 `data/uploads/<upload_set_id>/` 아래에 저장했다. 이 방식은 단일 API 컨테이너에서 빠르게 동작하지만, 컨테이너 재생성·프로젝트 간 원천 자료 공유·대용량 원천 파일 반복 업로드에는 약하다. 사용자는 업로드 파일을 RustFS에 저장할 수 있는 옵션, admin UI 설정, 이미 로컬에 있는 파일의 RustFS 업로드, RustFS에 이미 저장된 파일 목록 재사용, 기본 무기한 보존, Docker RustFS 동시 기동과 live test를 요구했다.

RustFS는 S3 호환 object storage이며, 이 저장소의 로컬 API 포트 `9001`과 RustFS 콘솔 기본 예시 포트 `9001`이 충돌한다.
...
9. Docker script는 RustFS를 `kraddr-geo-rustfs` 컨테이너로 관리한다. 프로젝트 표준 포트는 S3 API `9003`, console `9004`를 기본으로 둔다.
10. Playwright e2e 검증은 Windows Node/브라우저에서 Chrome(`chromium` project)과 Firefox(`firefox` project)를 모두 실행한다.
...
- RustFS console port를 `9004`로 분리하면 ADR-042의 API `9001` 원칙과 충돌하지 않는다.
...
- Docker, API, UI, 테스트는 이 ADR의 저장소 선택 계약을 따른다.
- 운영자는 RustFS data dir과 access key를 `.env`/secret manager에서 관리하고, 저장소에는 실제 secret을 커밋하지 않는다.

### 남은 위험

- RustFS image/env 계약은 RustFS 릴리스에 따라 달라질 수 있으므로 Docker script의 image, port, credential, data dir은 환경변수로 override 가능하게 둔다.
