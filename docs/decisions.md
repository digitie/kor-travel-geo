...
- `TH_SGCO_RNADR_LNBR.TXT`는 이번 PR에서 master에 쓰지 않고 행 수만 `DailyJusoLoadResult.unsupported_lnbr_rows`와 `load_manifest.source_set.unsupported_lnbr_rows`에 기록한다.
...
- `LNBR` 미반영으로 건물↔지번 1:N 관계 검색 정확도는 아직 개선되지 않는다. 이 범위는 ADR-022의 후속 T-038에서 구현한다.
...
`jibun_rnaddrkor_*`와 daily `LNBR`는 `tl_juso_text.pnu`에 덮어쓰지 않는다. 후속 구현에서 별도 테이블 `tl_juso_parcel_link`를 만든다.
...
- T-038 후속 작업에서 `tl_juso_parcel_link` DDL/Alembic, full snapshot loader, daily LNBR delta loader를 구현한다.
- T-028 daily loader가 기록하는 `unsupported_lnbr_rows`는 T-038 이후 실제 적용 건수로 대체한다.
