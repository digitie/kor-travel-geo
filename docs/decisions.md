...
본 문서는 `kor-travel-geo` / `kor-travel-geo-ui` 프로젝트의 의사결정을 시간순으로 누적한다. 결정이 뒤집힐 때도 이전 기록은 지우지 않고 `superseded by ADR-XXX`로 표시한다.
...
## ADR-014: 기본 예외명은 `KorTravelGeoError`로 둔다
...
### 컨텍스트
초기 사양에는 base 예외명이 `AddrKrError`로 적혀 있었다. 그러나 현재 패키지 식별자는 `kortravelgeo`로 확정되었고, 아직 공개 릴리스 전이라 외부 catch 코드와의 호환성 부담이 낮다.

### 결정
base 예외명은 `KorTravelGeoError`로 둔다. 장기 호환 alias는 만들지 않는다.
...
### 결과(긍정)
- 예외 계층이 `kortravelgeo` 식별자와 맞는다.
- `AddrKrError`/`KorTravelGeoError` 혼용을 피한다.
...
## ADR-015: `kortravel`는 implicit namespace package로 둔다
...
### 컨텍스트
이 저장소는 `kortravelgeo` 서브패키지를 제공한다. 향후 같은 환경에 `kortravel.tour` 같은 다른 `kortravel.*` 패키지가 설치될 수 있다. `src/kortravel/__init__.py`를 두면 PEP 420 namespace 병합을 막아 충돌 가능성이 생긴다.

### 결정
`src/kortravel/__init__.py`를 두지 않고 `kortravel`를 PEP 420 implicit namespace package로 둔다.

### 근거
- 여러 배포 패키지가 `kortravel.*` 하위 이름을 공유할 수 있다.
- parent package 소유권을 이 저장소가 독점하지 않는다.
- setuptools는 namespace package discovery(`namespaces = true`)로 `kortravelgeo`를 패키징한다.

### 결과(긍정)
- 향후 `kortravel.*` 패키지와 같은 Python 환경에서 공존하기 쉽다.
...
3. `kor-travel-geo-ui`: host `13088`, Next.js 내부/container `3000`.
...
- `.env.example`, README, `kor-travel-geo-ui/README.md`, `docs/ports.md`, `docs/dev-environment.md`가 같은 접속 설정 표를 따른다.
...
1. PostgreSQL + PostGIS는 `KTG_PG_DSN` 접속 설정을 사용한다.
2. FastAPI 백엔드 host/container 포트는 `9001`을 기본으로 둔다.
3. `kor-travel-geo-ui` host/container 포트는 `9002`를 기본으로 둔다.
...
- `docker/api.Dockerfile`, `kor-travel-geo-ui/Dockerfile`, `scripts/docker_app.sh`, Playwright 설정, UI proxy 기본값, README와 현재 운영 문서가 `9001`/`9002`를 따른다.
...
4. `kor-travel-geo-ui` host/container 포트는 `12205`이다.
...
- API 컨테이너는 기본 DSN `postgresql+psycopg://addr:addr@host.docker.internal:5432/kor_travel_geo`와 RustFS endpoint `http://host.docker.internal:12101`을 사용한다.
...
- 상태: proposed (T-123 평가 후 accepted 전환 보류)
- 날짜: 2026-06-14
- 결정자: codex
...
2. `/v1/address/geocode` 정상 응답은 `{"response": ...}`로 감싼다. `response.service.name`은 `address`, `response.service.operation`은 `getCoord`로 직렬화한다.
3. `/v1/address/reverse` 정상 응답도 `{"response": ...}`로 감싼다. `response.service.name`은 `address`, `response.service.operation`은 `getAddress`로 직렬화한다.
...
7. v1 geocode/reverse의 요청 검증·도메인 에러는 VWorld식 `response.error.level/code/text`로 반환한다. 대표 매핑은 `PARAM_REQUIRED`, `INVALID_TYPE`, `INVALID_RANGE`, `OVER_REQUEST_LIMIT`, `SYSTEM_ERROR`다.
...
그럼에도 C11은 향후 T-137/T-119에서 다시 논의될 수 있으므로, active serving 구현 전에 좌표 출처 노출 계약을 확정해야 한다. 특히 `pt_source`에 `c11_bundle_guarded` 같은 새 값을 넣을지, v1 VWorld 호환 표면을 어떻게 유지할지, v2 후보 metadata로 분리할지를 정해야 한다.
...
- (open) T-137에서 C11 최종 gate와 ADR-051 상태를 재판정한다.
- (open) T-119가 승인되면 구현 PR에서 `coord_source_detail` 산출, v1 geocode `x_extension`, v2 metadata/point_precision 매핑, OpenAPI/typegen/tests를 함께 반영한다.
