...
파일: `src/kortravelgeo/loaders/text/parcel_link_loader.py`
...
```bash
ktgctl load parcel-links data/juso/202603_도로명주소 한글_전체분 --yyyymm 202603
```
...
`juso_parcel_link_load`는 `tl_juso_text` FK에 의존하므로 반드시 `juso_text_load` 뒤에 실행한다. `kor-travel-geo-ui`의 `/admin/load` 기본 payload도 `juso_parcel_link_load`를 같은 도로명주소 한글 전체분 경로로 보낸다.
...
- `tests/integration/test_optional_real_postgres_load.py`
  - `KTG_TEST_PG_DSN`이 있으면 실제 Docker PostgreSQL에서 `tl_juso_parcel_link` snapshot/daily delta를 COPY/UPSERT한다.

수동 Docker 검증:

```bash
dropdb -h localhost -p 15432 -U addr --if-exists kor_travel_geo_t038
createdb -h localhost -p 15432 -U addr kor_travel_geo_t038
KTG_TEST_PG_DSN='postgresql+psycopg://addr:addr@localhost:15432/kor_travel_geo_t038' \
