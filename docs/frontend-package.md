...
`maplibre-vworld`는 `git+https://github.com/digitie/maplibre-vworld-js.git#11321fe`로 고정한다. 이 upstream commit은 PR #6/#7 merge 이후 `dist/`, package `exports`, `types`, `style.css`, zod v4 peer dependency를 포함하고, PR #9에서 추가한 `onMapClick`/`onMapError`/`flyToOptions`와 tile error helper를 제공한다. CI에서 SSH key 없이 설치되어야 하므로 lockfile의 `resolved`도 `git+https`를 유지한다. 패키지 SHA를 바꿀 때는 `npm ci`, `npm run type-check`, `npm run test`, `npm run build`로 소비자 build가 유지되는지 먼저 확인한다.
...
현재 `kraddr-geo-ui`는 upstream helper와 CSS를 실제 package dependency로 소비하지만 `VWorldMap` 컴포넌트 전체를 곧바로 대체하지는 않는다. 대신 VWorld tile 오류 분류와 URL redaction은 upstream `isVWorldTileError()`/`redactVWorldTileUrl()` helper로 맞춘다. 디버그 화면은 범용 지도 표시 외에 다음 동작을 보장해야 한다.
...
- **의존성 선언 상태**: `maplibre-vworld`는 `git+https://github.com/digitie/maplibre-vworld-js.git#11321fe`로 선언되어 있으며, `kraddr-geo-ui/lib/vworld.ts`는 upstream helper 재수출만 담당한다.
