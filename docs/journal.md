...
**T-297 디스크 확보 (같은 날, T-292 머지 직후)**: `docker system df`로 실측하니 n150의
디스크 압박 대부분이 이 프로젝트가 아니라 이 호스트를 공유하는 다른 프로젝트들의
Docker 이미지·빌드 캐시 누적이었다 — 예: `pinvi-pr477-stage-*` 스테이징 이미지가
서로 다른 커밋 해시로 40개+ 쌓여 있었고(각 ~2GB, 정리 없이 매 빌드마다 누적된 것으로
보임), 이미지 165.6GB 중 110.8GB·빌드 캐시 61.4GB 전부·볼륨 33.17GB 중 32.31GB가
reclaimable로 잡혔다. 가장 안전한 것부터 순서대로 처리 — `docker builder prune -af`
(빌드 캐시, 순수 재생성 가능이라 무위험) 먼저 실행해 61.4GB 회수(85%, 68G 여유)로
즉시 위험을 해소한 뒤, 사용자에게 더 진행할지 물어 확인받고 `docker image prune -af`
(어떤 컨테이너도 참조하지 않는 이미지만 삭제 — 실행 중/정지된 컨테이너가 쓰는 이미지는
전혀 건드리지 않음)까지 실행해 **최종 58% 사용(189G 여유)**으로 회복했다. 볼륨은
미참조분이 32GB 넘게 잡혔지만 다른 프로젝트의 실 데이터일 가능성이 있어 이번엔 손대지
...
VWorld API 키 반환. `/api/proxy/*`만 401이었다.

**1차 진단(맞음, 그러나 불충분)**: 폐기 목록이 Node in-process `Map`인데 `middleware.ts`는 Edge
런타임이라 못 본다. → Node에서 재검증하는 가드(`lib/session-guard.ts`)와 `admin/debug` 레이아웃,
`runtime-config` 검증을 추가했다.

**적대 리뷰가 잡은 것 — 1차 수정은 실제로 동작하지 않았다**: 리뷰어가 `next build && next start`
프로덕션 빌드로 재현해 `/admin`이 여전히 **200**임을 증명했다. 원인은 Edge/Node 경계가 아니라
**번들러 레이어별 모듈 인스턴스**였다 — turbopack이 `lib/auth.ts`를 라우트 핸들러 레이어(module id
45049)와 페이지 SSR 레이어(id 5246)로 각각 내보내 `const revokedSessionIds = new Map()`가 두 벌
존재했다. 라우트 핸들러끼리는 id가 같아 `/api/runtime-config`·`/api/proxy`만 우연히 동작했던 것.

**최종 수정**: 폐기 목록(과 로그인 실패 버킷)을 `globalThis`에 고정해 같은 프로세스의 모든 레이어가
한 Map을 공유한다. 여기에 Node 재검증 가드가 얹혀 페이지·키 반환 라우트가 모두 막힌다. middleware가
matcher(`\.js$` 부정 선읽기)에서 건너뛰던 `[report_id]` 동적 경로도 가드가 덮는다. 재로그인 시
`?next=` 보존을 위해 middleware가 요청 경로를 `x-ktg-pathname`으로 전달한다(값은 `sanitizeLocalPath`를
거치므로 open-redirect 불가).

**검증**: 프로덕션 빌드 재현 스크립트로 **수정 전 4/10 → 수정 후 10/10**. 이 결과를 영구 테스트로
`tests/e2e/live/session-revocation-live.spec.ts`(LIVE_E2E 게이트)에 남겼다. **유닛 테스트로는 이
...
자연스러운 후속이며, 그때는 백엔드 오류 시 **fail-closed**여야 한다(현재 rate-limit 경로는 fail-open).
...
서비스 전체를 기다리는 compose 재생성 요청은 중단하지 않고, UI 컨테이너가 이미 제거된 경우
`--no-deps` UI 단독 생성으로 즉시 복구했다. 컨테이너 이름도 관리 도구가 기대하는 표준 이름으로
정규화했고, API와 UI는 모두 healthy 상태다. 이 재발 방지 절차는 로컬 배포 runbook에 기록했다.

**라이브 검증**: UI 로그인 페이지와 API health가 200, 동일 Origin의 잘못된 로그인은 401,
UI/API의 admin proxy secret 일치와 역할이 필요한 admin API 200을 확인했다. UI 컨테이너의
시크릿·해시 환경 길이도 유효했다. 운영 노드 Chromium에서 컨테이너 시크릿으로 만든 일회성
세션(비밀번호를 읽거나 변경하지 않음)으로 `/admin` 200·`관리 홈` 렌더·Geocode 메뉴 이동·런타임
오류 0·1280px 가로 오버플로 0을 검증했다. 실제 평문 관리자 비밀번호가 배포 노드에 없으므로
정상 비밀번호를 넣는 로그인 POST 200만 운영자가 별도로 확인할 수 있다.
