...
이번 PR에서 `scripts/benchmark_mv_refresh.py`를 추가했다. 이 스크립트는 `KTG_PG_DSN`이 가리키는 DB에서 전략별 refresh를 실행하고 JSON으로 다음 값을 출력한다.
...
- `wait_events_before/after`는 같은 DB의 `wait_event IS NOT NULL` 전체 snapshot이다. 정상 idle backend의 `Client:ClientRead`도 들어올 수 있으므로, 활성 I/O 대기와 idle 연결을 분리해 해석해야 한다.
...
TMPDIR=/tmp TMP=/tmp TEMP=/tmp .venv/bin/python -m ruff check scripts/benchmark_mv_refresh.py tests/unit/test_mv_refresh_benchmark.py tests/unit/test_postload_mv.py src/kortravelgeo/loaders/postload.py
TMPDIR=/tmp TMP=/tmp TEMP=/tmp .venv/bin/python -m mypy scripts/benchmark_mv_refresh.py src/kortravelgeo/loaders/postload.py
```
