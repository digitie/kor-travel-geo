...
> **2026-08-17 갱신**: 아래의 PostgreSQL 접속 예시는 작성 당시 통합 인스턴스 `localhost:5432` 기준이다. manager
> ADR-37(PR #176)부터 geo DB는 전용 인스턴스 **`127.0.0.1:12500`**이고 `5432`를 듣는 것은 없다 — 접속할 때는
> `docs/ports.md`의 현재 값으로 host/port만 바꿔 읽는다(DB 이름·prefix·경로 규칙은 그대로 유효).
