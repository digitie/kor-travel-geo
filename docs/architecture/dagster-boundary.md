...
- **관측(read)**: API `/v1/ops/dagster/summary`·`/v1/ops/dagster/runs/{run_id}` — Dagster webserver에
  ...
  경로), **Dagster down이어도 200 + `status=unavailable`**(UI가 outage 렌더). 백엔드 GraphQL POST는
  **내부** `dagster_url`(SSRF allowlist)로 나가지만, 요약 응답의 **브라우저 노출 `dagster_url` 필드**는
  공개 URL `dagster_public_url or dagster_url`을 반환한다(T-290 — 아래 §8 iframe).
- **트리거(write)**: API → Dagster 방향은 `launchRun` mutation(selector=jobName/repository/location,
  ...
  그대로 사용한다. 온램프는 **최소권한 `scheduler` role**만 제시하고(actor `system:dagster`), API의
  `POST /v1/admin/backups/scheduled/run-due`는 `require_role(scheduler, destructive_admin)`으로 게이트한다
  (#429/ADR-066 — 공유 시크릿 시스템 액터에 파괴 범위를 주지 않는다). 파괴적 온램프(예: 향후 restore)만
  `DagsterAdminApiClient(roles=(destructive_admin,))`을 명시적으로 구성한다.
  - **선결 조건 (peer-trust / GeoIP)**: 컨테이너 토폴로지에서 Dagster 컨테이너의 peer IP는 loopback이
    아니다. 따라서 `KTG_ADMIN_TRUSTED_PROXY_CIDRS`에 Dagster 네트워크 CIDR을 추가하지 않으면
    `resolve_request_context`/`_peer_is_trusted`(`api/security.py`) + ADR-037 GeoIP gate가 온램프 호출을
    **403**으로 막는다. 실제 CIDR은 M2 배포 시점에 적용한다(여기서 배포 설정을 바꾸지 않는다).
- API 설정 키(env prefix `KTG_`): `dagster_url`(기본 `http://127.0.0.1:<port>`, **백엔드 GraphQL 관측
...
임베드**(`sandbox="allow-scripts allow-forms allow-popups allow-downloads allow-same-origin"`).
`allow-same-origin`은 Dagster SPA가 자기 origin의 `/graphql`·localStorage에 접근하기 위해 필요하며, 프레임
origin(Dagster host)이 부모 UI(:12505)와 cross-origin이라 부모 문서에는 접근하지 못해 안전하다. 요약 DTO는
at-a-glance, iframe이 full 제어면. iframe `src`와 '새 창' 링크·run 링크는 요약 응답의 **브라우저 노출
`dagster_url`**(= `dagster_public_url`, prod=라우터 공개 도메인 `https://geo-dagster.digitie.mywire.org/`)을
사용하고, 백엔드 관측/GraphQL은 **내부 `dagster_url`**(loopback, SSRF allowlist)을 유지한다(T-290 — 브라우저가
내부 loopback을 로드하려다 깨지는 문제를 분리로 해결). 기존 `/admin/backups`·`/admin/ops` progress는
`load_jobs`에서 계속 표시.
