...
| 테이블 | TanStack React Table v8 + TanStack React Virtual | 공용 `VirtualTable`이 필터·정렬·컬럼 정의를 맡고, 대량 목록은 row windowing을 쓴다 |
...
관리 UI의 표 표면은 native table 직접 구현이 아니라 `kor-travel-geo-ui/components/ui/VirtualTable.tsx` 공용 컴포넌트로 통일한다. 이 컴포넌트는 `@tanstack/react-table`로 컬럼 정의, 전역 필터, 정렬을 처리하고, `as="grid"` 모드에서는 `@tanstack/react-virtual`로 row windowing을 적용한다. 작은 목록이나 접근성상 실제 표 구조가 더 적합한 화면은 같은 컴포넌트의 `as="table"` semantic table 모드를 사용한다.
...
T-162 이후 런타임 예열은 API lifespan의 opt-in 백그라운드 작업과 수동 script로 분리한다. `loaders.runtime_warm`은 `pg_prewarm` extension과 서빙 relation 존재 여부를 먼저 확인하고, 선택적 `pg_prewarm`과 geocode exact/search text/reverse nearest/region radius 읽기 전용 probe를 상한 있는 쿼리로 실행한다. 기본값은 비활성이며, 여러 API worker가 동시에 예열을 실행하지 않도록 `RUNTIME_WARM` advisory lock을 사용한다. 재기동 직후 cold REST benchmark와 예열 후 REST benchmark는 `scripts/evaluate_t162_cold_warm_ratio.py`로 p99 비율 gate를 남긴다.

T-144 이후 v2/API 계약은 현재 성능 기준 profile을 유지한다. Geometry는 `include_geometry=false` 기본값으로 제외하고, v2 라우터는 `response_model_exclude_none=True`로 null field를 제거하며, geocode/search 후보 상한은 100으로 둔다. 새 field slim mode, geometry 분리 endpoint, detail expansion, pre-shaped response table은 즉시 도입하지 않고 T-105/T-139에서 근거가 생길 때 별도 breaking change로 다룬다. REST benchmark payload/p99 예산은 `scripts/evaluate_t144_api_contract.py`로 판정한다.
...
`soak_guard` artifact로 고정한다. T-164는 baseline/current `matrix-report.json`의 같은
`profile_id` p99를 비교해 입력분포 변화 회귀 gate를 만든다. PostgreSQL server 자원과
외부 REST worker 자원은 runner process guard 범위 밖이므로 운영 관측과 함께 해석한다.
...
- Prometheus는 애플리케이션이 직접 연결하지 않고 외부 scraper가 가져가는 pull 구조다. `kor-travel-docker-manager` 기준 관측 host 포트는 Grafana `12205`, cAdvisor `12301`, Prometheus `12401`이며 scrape target은 compose 내부 `kor-travel-geo-api:12501/metrics`, `kor-travel-geo-ui:12505/api/metrics`다. 같은 manager 기준으로 PostgreSQL은 geo 전용 인스턴스 `12500`(ADR-37, 2026-08-17), RustFS API/console은 `12101`/`12105`를 사용한다.
