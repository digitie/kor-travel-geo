# T-055: N150 / Odroid 운영 환경 검토

## 상태

- 상태: 측정 plan (하드웨어 도착 대기)
- 대상 브랜치: `agent/<agent>-t055-*`
...
- Docker PostGIS 16+3.5 또는 17+3.5 두 옵션 비교.
- `/usr/bin/time -v` 출력 + dstat/iostat 1초 간격 sample.

### 2. Serving latency (T-047 corpus 활용)

`scripts/benchmark_query_perf.py`(T-047)를 같은 corpus로 두 환경에서 실행:

- 8 query 군 × 4 concurrency (1, 4, 16, 64) × 30 iterations.
...
- `system-envelope.md`: OS/CPU/메모리/NVMe/Docker 버전 + fio/sysbench 결과.
- `fullload-phases.csv`: phase × env × wall_clock_seconds × max_rss_bytes.
- `serving-bench.json`: T-047 corpus 결과(`scripts/benchmark_query_perf.py` JSON 형식).
...
- `scripts/benchmark_query_perf.py` (T-047)와 `scripts/fullload_test.sh`(T-027 보강 후) 그대로 사용 가능한지 점검.
- Ubuntu 26.04 + Docker PostGIS 16+3.5 호환성 사전 확인 (현재 WSL2도 같은 image이지만 native Linux와 차이 가능).
- Docker rootless mode 사용 여부 결정.

## 검증 기준

- 두 환경에서 T-027 fullload_test.sh 1회 완주(end-to-end).
- T-047 benchmark corpus 1 round (30 iterations × 4 concurrency × 8 query 군).
