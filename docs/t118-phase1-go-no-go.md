...
> T-137 갱신(2026-06-16): T-129~T-134와 T-220/T-221을 종합한 최종 gate에서도 C11은 active serving 조건부 go가 아니다. Guarded policy는 검증 후보로만 보존하고, C11은 validation-only로 고정한다. ADR-051은 accepted로 전환하지 않으며 T-119는 계속 보류한다.
...
T-123 이후 추가된 T-125 gate는 `docs/t125-c11-serving-preflight.md`에 별도 체크리스트로 둔다. 기존 대표점 impact, C3/C4/C6/C7 회귀, 성능 회귀, feature flag rollback, v1/v2 노출 정책 중 하나라도 artifact로 채워지지 않으면 ADR-051을 accepted로 전환하지 않는다.

## 다음 작업

T-118 완료 후 즉시 진행할 Codex 작업은 T-120이었다. 이후 T-137 최종 gate까지 완료됐고, C11은 validation-only로 고정됐다. T-119는 ADR-051 accepted와 사용자 명시 승인, 그리고 새 correctness·성능 증거가 없으면 진행하지 않는다.
