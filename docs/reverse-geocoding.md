...
역지오코딩은 `SpatialiteAddressStore`가 담당한다.

## 로컬 조회 흐름

1. 요청 좌표를 store SRID로 변환한다. 기본값은 EPSG:5179다.
2. `juso_address_points`에서 `x`, `y` 인덱스를 활용한 경계 상자 후보를 찾는다.
3. 평면 거리와 `source_priority`를 기준으로 후보를 정렬한다.
4. 도로명주소, 법정동 코드, 우편번호, 좌표 역할, 원천 데이터셋을 담은 `ReverseGeocodeResult`를 반환한다.
...
로컬 후보를 찾지 못했고 VWorld 인증 정보가 설정되어 있으면 같은 메서드가 `python-vworld-api` 대체 호출을 사용할 수 있다.
