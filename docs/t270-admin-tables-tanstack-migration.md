...
상태: **완료** (Agent B/Claude) · 2026-06-17 · 사용자 지시: admin UI의 **모든 테이블**을
TanStack React Table + TanStack React Virtual 기반 공유 컴포넌트로 교체.
(T-270~T-276 머지 완료 — 아래 "완료 기록" 참조. T-276 e2e는 PR #327에서 추가.)
...
- SSE 행: per-row 훅을 테이블 밖 sibling `SessionLiveSubscriber`로 끌어올려(id keyed) VirtualTable 셀은 순수 유지.
- consistency 서버 페이징: VirtualTable 클라 검색/정렬 OFF + 외부 Pager 유지(가상화로 대체하지 않음).
- setmeta 전치 key/value: VirtualTable `hideHeader`+`rowHeader`(additive)로 헤더 없는 행-헤더 테이블 충실 전환.

## 완료 기록 (2026-06-17)

T-270~T-275 전부 머지(PR #321·#322·#323·#324·#325·#326). 27개 테이블 전부 공유 `VirtualTable`
기반(TanStack Table)으로 전환 완료. wire/백엔드 영향 없음.

### T-276 테스트 플랜 재점검 (e2e 실행 전)
디스커버리 결과를 실제 코드에 비춰 재점검한 결론:
- **matchset-compare `tr` 단언 업데이트 불필요**: `MatchSetComparePanel`을 `as="table"`(semantic `<tr>`)로
  전환했으므로 기존 `locator('tr',{hasText})` 단언이 변경 없이 통과 → 회귀 spec 수정 0건.
- **정렬 토글/패널-scoped 검색+count**: 대부분 표가 `as="table"` 정적(검색/정렬 toolbar 없음)이라
  해당 신규 e2e 불필요. 검색이 있는 곳은 backups artifact(grid, 기존 `backups.spec.ts`가 이미 커버),
  TableStatsPanel은 단일 패널이라 '목록 검색' strict-mode 중복 없음 → 별도 신규 불필요.
- **grid windowing(DOM<<total)**: `VirtualTable` grid 모드의 windowing은 `virtual-table.test.tsx`
  단위에서 jsdom 가상화로 커버. 대용량 fixture e2e는 비용 대비 이득 낮아 보류(필요 시 후속).
- **실 공백**: OpsPanel 7개 표는 기능 e2e가 전무했음(ops-a11y는 빈 배열) → **신규 `ops.spec.ts` 추가**.

### 신규/검증
- 신규 단위: `virtual-table.test.tsx` +5 (semantic table caption/scope/columnheader/row/cell, table 정렬·aria-sort,
  emptyHint colspan, headerCell+active-row+onRowClick, align, hideHeader, rowHeader) → 총 118.
- 신규 e2e: `tests/e2e/ops.spec.ts` (2) — 7개 ops 테이블 caption a11y·행/셀·emptyHint.
- 회귀: 기존 e2e 무수정 통과(`source-files-*`, `backups*`, `consistency*`, `ops-perf-summary`, `restore`, `hotswap`).

### e2e 실행 결과 (Windows Playwright, chromium, fixture-mock, port 12505)
`npx playwright test` → **85 passed, 2 failed**.
- 2건 실패는 **환경 의존 통합 spec**으로 본 작업과 무관:
  - `navigation.spec.ts`: 실 Python 백엔드 미기동 → proxy 500(페이지 에러). 백엔드 없는 로컬에서 항상 실패.
  - `vworld-map.spec.ts`: 실 VWorld API 키(.env) 필요. 로컬 미설정.
- 나머지 마이그레이션 surface 전부(85건) green. 단위 vitest 118, type-check/lint/build green.
