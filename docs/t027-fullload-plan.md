...
- 2026-06-01에는 사용자 지시에 따라 별도 Docker DB `kor-travel-geo-t027-retune`(port `15435`)에서 T-027 전체 적재를 다시 수행하고, 20260401~20260506 daily ZIP 전체를 실제 데이터로 적용했다. 최종 `mv_geocode_target=6,418,735`, `mv_geocode_text_search=6,418,735`, `tl_sppn_makarea=24,204`이며 상세 비교는 `docs/t027-t047-sppn-retune-20260601.md`에 기록했다.
- 대상 PR: T-027 최종 클린 적재 보강 PR
- 실행 대상 데이터: `F:\dev\kor-travel-geo\data\juso` (`/mnt/f/dev/kor-travel-geo/data/juso`)
...
2026-05-24 기준 로컬 경로를 읽어 확인한 결과, `data/juso` 전체 용량은 약 28GB다. 2026-05-29 최종 클린 재적재에서는 NTFS 원본을 WSL ext4 작업 사본 `/home/digitie/kor-travel-geo-data`로 복사한 뒤 Docker PostGIS(`localhost:15434`)에 새 compose project/pgdata를 만들어 처음부터 적재했다.
...
- `KTG_PG_DSN`이 새로 준비된 검증 DB를 가리킨다는 점 확인
...
이 저장소는 PostgreSQL/PostGIS를 직접 구동하지 않는다. 전체 적재 검증은 이미 동작 중인 PostgreSQL/PostGIS에 `KTG_PG_DSN`으로 접속해 수행한다.

| 항목 | 값 |
|------|----|
| DB | `kor_travel_geo` |
| 사용자/비밀번호 | `addr` / `addr` |
| 기본 DSN 예시 | `postgresql+psycopg://addr:addr@localhost:15434/kor_travel_geo` |

운영성 데이터와 섞이지 않게 별도 DB를 준비하고, 이 저장소에는 해당 DB의 DSN만 설정한다.
...
```bash
export DATA_DIR=/mnt/f/dev/kor-travel-geo/data
export JUSO_YYYYMM=202603
export LOCSUM_YYYYMM=202604
export NAVI_YYYYMM=202604
export DAILY_JUSO_ZIP=/mnt/f/dev/kor-travel-geo/data/juso/daily/20260401_dailyjusukrdata.zip
...
export KTG_PG_DSN=postgresql+psycopg://addr:addr@localhost:15434/kor_travel_geo
...
PY
python -c "import kortravelgeo; print(kortravelgeo.__version__)"
```

### Phase 1: DDL 적용

`ktgctl init-db` 명령을 사용한다. `SCHEMA_SQL` → `INDEX_SQL` → `MV_SQL`을 순서대로 적용하므로 빈 MV와 unique index까지 준비된다. 이미 존재하는 인덱스/MV는 경고만 출력하고 계속 진행한다.
...
```bash
export KTG_DB_PORT=15432
export KTG_PG_DSN=postgresql+psycopg://addr:addr@localhost:${KTG_DB_PORT}/kor_travel_geo
ktgctl init-db
```
...
export KTG_PG_STATEMENT_TIMEOUT_MS=1800000

bash scripts/fullload_test.sh 2>&1 | tee artifacts/fullload_$(date +%Y%m%d_%H%M%S).log
```

`KTG_PG_STATEMENT_TIMEOUT_MS=1800000`은 30분이다. 운영 API 기본값 5초는 일반 조회를 보호하기 위한 값이므로, 전국 풀로드의 링크 해소·shadow MV 생성처럼 대량 scan/sort가 필요한 유지보수 작업에는 별도 timeout을 사용한다. `scripts/fullload_test.sh`는 이 값이 없으면 30분을 기본값으로 export한다.

스크립트 내부에서 실행되는 핵심 명령:

```bash
ktgctl load juso   "$DATA_DIR/juso/${JUSO_YYYYMM}_도로명주소 한글_전체분" --yyyymm "$JUSO_YYYYMM"
ktgctl load locsum "$DATA_DIR/juso/${LOCSUM_YYYYMM}_위치정보요약DB_전체분.zip" --yyyymm "$LOCSUM_YYYYMM"
ktgctl load navi   "$DATA_DIR/juso/${NAVI_YYYYMM}_내비게이션용DB_전체분" --yyyymm "$NAVI_YYYYMM"
```

T-039 direct 출입구는 선택 적재다. 같은 기준월 자료로 동월 검증을 구성했거나 기준월 혼합을 의도적으로 기록하려는 경우에만 다음 명령을 추가한다.

```bash
ktgctl load roadaddr-entrances \
...
```bash
ktgctl load daily-juso "$DAILY_JUSO_ZIP"
ktgctl load daily-parcel-links "$DAILY_JUSO_ZIP"
```
...
별도 `load juso/locsum/navi/shp-all` 명령을 순서대로 실행한 경우에는 `bd_mgt_sn` 후처리를 반드시 별도로 수행해야 한다. PR #13 스크립트는 `resolve_text_geometry_links()`를 직접 호출한 뒤 full-load에 더 적합한 `ktgctl refresh mv --swap`을 사용한다. 복구 과정에서 기존 `mv_geocode_target`이 없어진 경우에도 `refresh mv --swap`은 `mv_geocode_target_next`를 바로 운영 이름으로 승격할 수 있어야 한다.
...
부분 적재 결과를 버리고 새로 시작해야 하면 외부 인프라 관리 경로에서 새 빈 DB를 준비하고, `KTG_PG_DSN`을 그 DB로 바꾼다.
...
| 데이터 작업 사본 | `/home/digitie/kor-travel-geo-data` |
| daily ZIP | `/mnt/f/dev/kor-travel-geo/data/juso/daily/20260401_dailyjusukrdata.zip` |
| PostgreSQL | PostGIS `16-3.5`, `localhost:15434`, DB `kor_travel_geo` |
| 전용 pgdata | `/home/digitie/kor-travel-geo-data/pgdata-final-20260529` |
| OS/WSL | Ubuntu 24.04 on WSL2 |
| CPU/RAM | AMD Ryzen 7 7840HS, 16 vCPU, RAM 약 29GiB |
| GDAL/Python | GDAL 3.8.4, Python 3.12.3 |
| 시작 디스크 | ext4 `/dev/sdd` 약 1007GiB 중 737GiB 여유 |

기존 `localhost:15432` DB는 건드리지 않고, 별도 `localhost:15434` DB를 사용했다. `load shp-all --mode full`의 TRUNCATE 직전 row count가 9개 SHP 테이블 모두 0이어서 빈 DB에서 시작했음을 확인했다.
...
env \
  ...
  DAILY_JUSO_ZIP=/mnt/f/dev/kor-travel-geo/data/juso/daily/20260401_dailyjusukrdata.zip \
  PYTHON=/home/digitie/.cache/kor-travel-geo-venv/bin/python \
  KTG_BIN=/home/digitie/.cache/kor-travel-geo-venv/bin/kor-travel-geo \
  KTG_PG_DSN=postgresql+psycopg://addr:addr@localhost:15434/kor_travel_geo \
...
`ktgctl validate data-quality-samples --cases C2,C4,C6,C7 --limit 20`을 `KTG_PG_STATEMENT_TIMEOUT_MS=1800000`로 실행해 CSV 8개를 생성했다. 첫 실행은 기본 statement timeout에서 C4 temp table 생성 중 `QueryCanceled`로 실패했으므로, 전국 data-quality export도 full consistency와 같은 유지보수 timeout으로 실행해야 한다.
...
| 데이터 작업 사본 | `/home/digitie/kor-travel-geo-data` |
| 원본 데이터 | `/mnt/f/dev/kor-travel-geo/data/juso` |
| PostgreSQL | Docker PostGIS, `localhost:15432`, DB `kor_travel_geo` |
| OS/WSL | Ubuntu 24.04 on WSL2 |
| CPU/RAM | AMD Ryzen 7 7840HS, 16 vCPU, RAM 약 29GiB |
...
`ktgctl validate data-quality-samples --cases C2,C4,C6,C7 --limit 20`을 실행해 CSV 8개를 생성했다. 경과는 86.18초, 최대 RSS는 82,292KB다.
...
| 작업 디렉터리 | `/home/digitie/dev/kor-travel-geo` |
| 데이터 작업 사본 | `/home/digitie/kor-travel-geo-data` |
| Docker compose project | `kor-travel-geo-t027` |
| PostgreSQL container | `kor-travel-geo-t027-db-1` |
...
SHP 9개 테이블 `ANALYZE` 후 `ktgctl validate consistency --scope full`을 실행했다.
...
- `python -m kortravelgeo.cli ...` 대신 설치된 console script `kor-travel-geo ...`를 사용한다.
