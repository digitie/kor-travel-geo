...
| `daily/*.zip` | 일변동 ZIP 20260401~20260506 | 지원 | T-028 이후 `TH_SGCO_RNADR_MST.TXT`는 `tl_juso_text` 증분 적용 가능. T-038 이후 `TH_SGCO_RNADR_LNBR.TXT`는 `tl_juso_parcel_link` delta source로 적용 가능 |
| `건물군 내 상세주소 동 도형/` | 시도별 ZIP | 분석 지원 | T-041 이후 `scripts/compare_extra_shape_layers.py`로 전자지도 건물과 비교 가능. serving loader는 보류 |
| `구역의 도형/` | 별도 도형 묶음 | 분석 지원 | T-041 이후 중복 5개 레이어와 추가 2개 레이어를 비교 가능. `TL_SPPN_MAKAREA`는 ADR-027에 따라 국가지점번호 보조 데이터 후보이나 아직 기본 full-load에는 미포함 |
...
정합성 C10은 기준월 불일치를 감지한다. ADR-029/T-045 이후에는 이 조합을 `source_set.yyyymm_by_kind`로 명시하고, 혼합 기준월임을 운영자가 확인한 경우 `mixed_yyyymm_acknowledged=true`로 남긴다. 확인 기록이 없는 혼합 기준월은 실수 가능성이 있으므로 `ERROR`로 막고, 확인된 혼합 기준월은 C10 `INFO` 또는 `WARN`과 note로 남긴다. "동월 전체분" 검증이 필요하면 도로명주소 한글 전체분도 `202604` 또는 direct 출입구와 같은 `202605` 자료로 맞춰 받아 재실행해야 한다. T-039 direct 출입구는 선택 적재로만 실행하고 기본 full-load batch 6종에는 넣지 않는다.

ADR-030/T-046 구현 이후 최종 클린 적재가 성공하면 즉시 `serving-ready` 백업을 생성한다. 이 백업은 `pg_dump -Fd --jobs` directory dump를 `tar.zst` artifact로 묶은 형태여야 하며, 복원 검증은 먼저 대구광역시 부분 DB에서 완료한 뒤 전국 DB에는 보존용 백업 생성만 수행한다.
...
| T-039 `도로명주소 출입구 정보` direct entrance loader | 완료. `tl_roadaddr_entrc` 선택 적재, MV 대표 좌표 1순위 후보, 기본 full-load 자동 포함 제외 |
...
| T-042 `TL_SPPN_MAKAREA` 국가지점번호 보조 데이터 | 대기. `tl_sppn_makarea` 별도 테이블, reverse/geocode enrichment, 디버그 UI overlay 설계를 구현한 뒤 최종 클린 로드에 포함 여부를 다시 판단 |
| T-047 전국 적재 후 쿼리 성능 벤치마크 | 대기. T-027 클린 DB에서 p95/p99 query latency를 측정하고, 목표 초과 시 index/query rewrite/보조 view·MV를 적극 실험 |
