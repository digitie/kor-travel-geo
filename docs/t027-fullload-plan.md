...
- 상태: 2026-05-27 최종 클린 적재 1회 완료. direct 출입구 same-month gate와 C10 row-level 기준월 집계를 보강했고, 보강 후 C1~C10/smoke 재검증까지 완료했다.
...
2026-05-24 기준 로컬 경로를 읽어 확인한 결과, `data/juso` 전체 용량은 약 28GB다. 2026-05-27 최종 클린 적재에서는 NTFS 원본을 WSL ext4 작업 사본 `/home/digitie/kraddr-geo-data`로 복사한 뒤 Docker PostGIS(`localhost:15432`)에 새 DB를 만들어 처음부터 적재했다.
