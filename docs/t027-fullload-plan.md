...
- `KRADDR_GEO_PG_DSN`이 새로 준비된 검증 DB를 가리킨다는 점 확인
...
이 저장소는 PostgreSQL/PostGIS를 직접 구동하지 않는다. 전체 적재 검증은 이미 동작 중인 PostgreSQL/PostGIS에 `KRADDR_GEO_PG_DSN`으로 접속해 수행한다.

| 항목 | 값 |
|------|----|
| DB | `kraddr_geo` |
| 사용자/비밀번호 | `addr` / `addr` |
| 기본 DSN 예시 | `postgresql+psycopg://addr:addr@localhost:15434/kraddr_geo` |
...
부분 적재 결과를 버리고 새로 시작해야 하면 외부 인프라 관리 경로에서 새 빈 DB를 준비하고, `KRADDR_GEO_PG_DSN`을 그 DB로 바꾼다.
...
| PostgreSQL | PostGIS `16-3.5`, `localhost:15434`, DB `kraddr_geo` |
