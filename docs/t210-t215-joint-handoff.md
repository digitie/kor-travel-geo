...
| **T-215** | ✅ 평가 완료 — geocode/reverse 정확도·v1(vworld)/v2 smoke, C1~C10 재실행, C11~C17 run-validation, SQL/REST c64 재측정. REST c64 tail과 C11~C17 optional source full run은 T-126으로 분리. N150/Odroid 실측은 T-063 연계 | T-214 + 실장비(T-063 보류) | A(Codex) |
...
T-214/T-215는 기본 개발 DB `kor_travel_geo`를 암묵적으로 쓰지 않는다. T-213 기준 데이터는 전용 PostgreSQL DB, 전용 RustFS prefix, NTFS `F:\dev\geodata\t213-baseline\<run-id>\` artifact 조합으로 보존해야 하며, preflight에서 active serving release와 row count가 summary와 일치하지 않으면 T-213을 전용 공간에 재실행한 뒤 진행한다. 현재 T-214/T-215 기준 baseline은 `kor_travel_geo_t213_20260615_r3`, RustFS `kor-travel-geo/t213/20260615-rerun3`, T-213 artifact `F:\dev\geodata\t213-baseline\20260615-rerun3\`, T-214 artifact `F:\dev\geodata\t214-benchmark\20260615-r3\`, T-215 artifact `F:\dev\geodata\t215-acceptance\20260615-r1\`, serving release `54e17e80-312e-46da-a58f-d8b10be37c85`다. T-214 결과는 SQL/REST error 0, SQL c64 worst p95 `Q4_SEARCH/search_fuzzy=245.895ms`, REST c64 worst p95 `Q4_SEARCH/search_fuzzy=534.031ms`, MV `concurrent=126.414s` / `swap-rerun=340.128s`, RustFS reconcile prefix orphan warning 2건이다. T-215 결과는 preflight와 v1/v2 smoke 통과, C1~C10 `severity_max=ERROR`(known data-quality), C11~C17 skipped 7건, SQL c64 worst p95 `Q4_SEARCH/search_fuzzy=308.617ms`, REST c64 sample worst p95 `Q3_FUZZY_GEOCODE/geocode_fuzzy_hint=3631.900ms`다. 상세는 `docs/t214-phase2-performance-benchmark.md`와 `docs/t215-phase2-final-acceptance.md`를 본다. T-213 입력 원천은 `F:\dev\geodata\juso`를 기준으로 두고, 사용하지 않는 원천은 `juso\unused\` 아래에 보존한다.

이 핸드오프 기준으로 Agent A(로더/실데이터)와 Agent B(신규 파이프라인, 완료)는 T-126에서 REST c64 tail과 C11~C17 optional source 검증을 이어가면 된다.
