...
| 항목 | 다음 처리 |
|------|-----------|
| `pg_stat_statements` | Docker/PostgreSQL 설정에서 extension을 활성화하고, benchmark run 전후 snapshot artifact를 저장한다. |
...
| pool wait/DB execution 분리 | `pg_stat_statements`, connection-preacquired run, EXPLAIN aggregate 중 하나 이상으로 wall time을 분해한다. |
