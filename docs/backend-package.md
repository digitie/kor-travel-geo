...
`건물군 내 상세주소 동 도형`, `구역의 도형`, `도로명주소 건물 도형`은 현재 full-load source child에 포함하지 않는다. T-030 실제 파일 검토 결과, 이 자료들은 기준월과 레이어 의미가 다르므로 기본 serving path에 즉시 섞지 않고 다음 후속으로 분리했다.
