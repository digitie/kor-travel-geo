...
- `POST /v1/admin/upload/sido-zip?filename=...&sido=...` — 시도 ZIP raw body 스트리밍 업로드(SHA256 해시 반환)
...
1. **업로드**: 시도별 ZIP을 `POST /v1/admin/upload/sido-zip?filename=...&sido=...`로 보낸다. 요청 body는 multipart가 아니라 raw binary stream이다. 이 방식은 `python-multipart` 의존을 추가하지 않고 FastAPI `Request.stream()`으로 바로 디스크에 쓰기 위함이다. 업로드된 경로/사이즈/sha256을 반환한다.
