...
본 문서는 첨부 사양서(2026-05-22 작성)를 `main` 브랜치 문서 체계로 옮긴 정리본이다. 구현 시 본 문서를 1차 reference로 본다. GitHub 저장소 이름은 `python-kraddr-geo`, Python import는 `kraddr.geo`, CLI는 `kraddr-geo`, 환경변수 prefix는 `KRADDR_GEO_`다.
...
- `GET  /v1/admin/ops/dataset-snapshots`, `GET /v1/admin/ops/dataset-snapshots/{id}` — source set, row count, consistency/performance/artifact 연결 상태 조회.
- `GET  /v1/admin/ops/serving-releases`, `POST /v1/admin/ops/serving-releases/{id}/activate` — active serving release와 rollback lineage 관리. active release는 DB 제약으로 한 건만 허용한다.
- `GET  /v1/admin/ops/artifacts`, `GET /v1/admin/ops/artifacts/{id}` — backup, restore log, consistency export, performance report, source inventory 공통 artifact metadata 조회.
- `POST /v1/admin/ops/maintenance-windows` — destructive restore, schema migration, full reset 같은 위험 작업을 위한 maintenance window 등록. typed confirmation 없이는 실행하지 않는다.
...
- `GET  /v1/admin/jobs/{id}/events` — 대형 관리 작업 진행률 SSE. T-046 UI는 SSE 우선, 실패 시 polling fallback을 사용한다.
...
- 핸들러 등록: `queue.register(kind, handler)`. 핸들러 시그니처는 `(payload, cancel_event, progress_cb)`이며, `progress_cb(progress?, stage?, message?)`를 호출하면 `load_jobs.progress`, `current_stage`, `heartbeat_at`, `log_tail`이 함께 갱신된다. 기본 앱은 `juso_text_load`, `daily_juso_delta`, `juso_parcel_link_load`, `juso_parcel_link_delta`, `roadaddr_entrance_load`, `locsum_load`, `navi_load`, `shp_polygons_load`, `pobox_load`, `bulk_load`, `consistency_check`, `mv_refresh` 핸들러를 등록한다. T-046/T-047 구현 후에는 같은 큐에 `db_backup`, `db_restore`, `query_benchmark` 핸들러도 등록한다. 이 중 `roadaddr_entrance_load`는 선택 child라 기본 `full_load_batch` 6종에는 들어가지 않는다.
...
구현 검증은 전국 full-load를 다시 실행하지 않고 대구광역시 부분 적재 DB로 수행한다. `kraddr_geo_t046_daegu`를 백업하고 `kraddr_geo_t046_daegu_restore`에 복원한 뒤 row count, `mv_geocode_target`, 대구 주소 geocode/reverse smoke test를 비교한다.
...
kraddr-geo backup create \
  --destination-dir /mnt/f/backups/kraddr-geo \
  --profile serving-ready \
  --jobs 4 \
  --callback-url http://localhost:9000/hooks/backup-complete
kraddr-geo backup list
kraddr-geo backup show <artifact_id>

# 복원은 기본적으로 새 빈 DB에만 수행한다.
kraddr-geo restore create \
  --artifact-id <artifact_id> \
  --target-database kraddr_geo_t046_daegu_restore \
  --jobs 4 \
  --run-smoke-test

# === 전국 DB 쿼리 성능 벤치마크 (ADR-031, T-047) ===
# full-load 이후 로컬 DB query latency를 반복 측정한다. 외부 API fallback은 baseline에서 끈다.
kraddr-geo benchmark queries \
