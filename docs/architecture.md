...
- 메트릭: `prometheus-client`. API `/metrics`에서 외부 API 호출 카운터, cache entries/hits/expired, 적재 작업 kind/state gauge, 적재 job/stage duration histogram, v1/v2 API 요청 total/duration/slow/in-flight/cancel, API admission wait/rejection/in-flight, SQLAlchemy DB pool gauge와 checkout timeout counter, SQL query operation/fingerprint/status별 duration histogram과 query cancel counter, persisted `pg_stat_statements` top-N gauge를 노출한다. query fingerprint는 원문 주소와 literal을 label로 노출하지 않기 위한 저카디널리티 식별자이며, `pg_stat_statements` metric도 query 원문 대신 `rank`/`operation`/`query_fingerprint`만 label로 사용한다.
