...
다음 항목은 권고안에 선택지가 있으므로 최종 구현 전에 별도 결정을 받아야 한다. 이 문서는 각 선택지의 장단점과 영향만 고정하고, 최종 선택은 후속 ADR 또는 구현 착수 전 사용자 결정으로 확정한다.

| 항목 | 선택지 | 장점 | 단점/비용 | 현재 문서 처리 |
|------|--------|------|-----------|----------------|
| C11+ case metadata | A. `CASE_DEFINITIONS` 정적 tuple 확장 | 현재 구조와 가장 가깝고 구현량이 작음 | C11+가 늘어날수록 배포 없이 case metadata 수정 불가 | 1차 구현 후보 |
| C11+ case metadata | B. DB registry 또는 설정 기반 동적 case catalog | UI가 C11+를 자연스럽게 동적 렌더링, 운영 중 확장 쉬움 | DTO/API/UI/검증 runner 변경량 큼 | 장기 후보 |
| match set과 release/snapshot 연결 | A. `ops.dataset_snapshots.source_match_set_id` FK 추가 | active release가 snapshot을 통해 안정적으로 match set을 찾음 | migration 필요, 복원 시 FK 재구성 정책 필요 | 우선 후보 |
| match set과 release/snapshot 연결 | B. `ops.dataset_snapshots.source_set` JSONB 안에 ID 저장 | migration이 작거나 불필요 | FK 무결성 없음, 조회와 정합성 검증이 약함 | 보조/복원 fallback |
| match set과 release/snapshot 연결 | C. `ops.serving_releases`에 직접 FK 추가 | 현재 release 화면에서 바로 표시 가능 | release는 snapshot보다 배포 상태에 가까워 원천 조합 정본 위치로 애매함 | 비권장 |
| source file 검증 상태 | A. `ops.source_files.state` 단일 상태만 사용 | 모순 상태가 줄고 UI 단순 | 최신 validation 세부 상태는 join/JSON에서 파생해야 함 | 결정 필요 |
| source file 검증 상태 | B. `state`와 `validation_state` 분리 | 보관 상태와 검증 상태를 독립적으로 표현 가능 | `available + validation_failed` 같은 모순 방지를 별도 CHECK/trigger로 강제해야 함 | 결정 필요 |
| commit 비원자성 처리 | A. storage-first 후 DB insert | object put 성공 후 DB row를 기록하는 흐름이 단순 | DB insert 실패 시 orphan object 발생 | reconciliation으로 보완 가능 |
| commit 비원자성 처리 | B. DB pending row 먼저 생성 후 staging object put, 검증 후 available 전환 | DB에서 모든 시도를 추적 가능, 실패 복구 쉬움 | pending row cleanup과 staging object lifecycle 필요 | 우선 후보 |
| commit 비원자성 처리 | C. staging key put 후 DB insert, 최종 key copy/delete | 사용자 노출 prefix에 partial object가 덜 남음 | S3 rename이 없어 copy/delete 비용, 대용량에서 비쌈 | 대용량 비용 검토 필요 |
| admin 권한 모델 | A. 기존 내부망/GeoIP/운영자 전제 + typed confirmation | 구현량 작고 현재 admin 구조와 맞음 | 파괴적 액션의 사용자별 권한 추적이 약함 | 최소 구현 |
| admin 권한 모델 | B. admin role gate 도입 | hard-delete/hash 갱신/rebuild 같은 위험 액션을 권한으로 분리 | 인증/세션/권한 설계가 별도 큰 작업 | 권장 구현 방향 |
| PK 네이밍 | A. full-prefix (`source_file_id`) 유지 | 테이블 간 ID 의미가 명확하고 API DTO와 잘 맞음 | 기존 ops의 `release_id`, `snapshot_id` 스타일과 다름 | 결정 필요 |
| PK 네이밍 | B. 짧은 어간 (`file_id`, `set_id`, `item_id`) 사용 | 기존 `ops.*` 일부 관례와 가까움 | 다른 테이블과 join할 때 의미가 흐릴 수 있음 | 결정 필요 |
| 시도별 다중 파일 category 모델 | A. `ops.source_file_groups` 신설, match set은 group 참조 | 17개 시도 coverage와 개별 file metadata를 모두 보존하고 FK도 명확함 | 테이블/API가 하나 늘어남 | 우선 후보 |
| 시도별 다중 파일 category 모델 | B. group 테이블 없이 `(category, user_yyyymm)`와 `sido_code`로 암묵 그룹화 | DDL이 적고 빠르게 구현 가능 | 중복 기준월 group, coverage, FK, 삭제 보호가 애매함 | 비권장 |
| 시도별 다중 파일 category 모델 | C. 실제로는 17개 파일이지만 가상 bundle source_file 1행으로 표현 | 기존 match set의 단일 `source_file_id` 모델을 유지 가능 | 파일명/hash/download/delete가 실제 archive와 어긋나고 구현자를 오도함 | 금지 |
| 업로드 전략 | A. 현행 단일 PUT 유지 | 현재 확인된 단일 archive와 시도별 ZIP 크기에서는 충분하고 구현량이 작음 | 네트워크 중단 시 전체 재시도, 향후 더 큰 원천에는 취약 | 1차 구현 |
| 업로드 전략 | B. multipart/resumable upload | 대형 archive와 네트워크 실패에 강함, percent 표시도 안정적 | API/UI/RustFS multipart 구현량 큼 | 후속 운영 hardening |
| RustFS hash 검증 | A. reconciliation 때 object 전체를 즉시 재해시 | hash mismatch를 즉시 확정 가능 | 대형 archive가 많으면 I/O와 egress 비용 큼 | 제한적 실행 |
| RustFS hash 검증 | B. 기본 scan은 size/etag, hash는 on-demand 재해시 | 정기 검사가 가볍고 운영 부담이 낮음 | hash mismatch 확정까지 사용자의 추가 액션 필요 | 우선 후보 |
...
| M3 | `ops.dataset_snapshots.source_match_set_id` FK 우선 후보와 JSON fallback을 명시 |
| M4 | `RustfsClient`의 `head_object`, `delete_object`, metadata 포함 `put_file`, staging/copy helper 필요를 구현 범위에 추가 |
...
| M8 | DB/RustFS 비원자성, advisory lock, active match set partial unique, orphan staging SLA를 추가 |
...
| L1 | SHP 3종은 시도별 개별 ZIP 업로드로 확정해 2GiB 장애 요인을 해소하고 multipart는 후속 운영 강화로 이동 |
...
| `epost_pobox_full` | epost 사서함 | 우편번호 보조 | 기존 `pobox` source kind / `pobox_load` job과 연결 |
| `epost_bulk_full` | epost 다량배달처 | 우편번호 보조 | 기존 `bulk` source kind / `bulk_load` job과 연결 |
...
현재 `kortravelgeo.core.consistency_definitions.CASE_DEFINITIONS` 정적 tuple과 UI는 C1~C10을 전제로 한다. optional 검증 자료가 늘어나면 C11 이상이 필요하다. 구현에서는 정적 tuple에 C11+를 추가할 수 있지만, 장기적으로는 DB/코드 registry에서 case metadata를 API로 내려 UI가 동적으로 탭을 그리게 해야 한다. 어떤 경로를 택하든 `ops.consistency_case_samples.case_code` CHECK는 현재 C1~C10만 허용하므로 `^C\d+$` 같은 패턴으로 완화하는 Alembic migration을 반드시 포함한다.
...
위 DDL은 `state`와 `validation_state`를 분리하는 B안 예시다. 최종 결정을 단일 상태 A안으로 내리면 group/file 양쪽의 `validation_state` 컬럼을 제거하고 최신 `ops.source_file_validations` row와 `validation_summary`에서 검증 상태를 파생한다. B안을 유지하면 `state='available'`일 때 `validation_state IN ('passed','warning')`만 허용하는 CHECK 또는 trigger를 추가해 모순 상태를 막아야 한다.
...
현재 active serving release와 match set 연결은 아직 최종 결정이 필요하다. 우선 후보는 `ops.dataset_snapshots.source_match_set_id UUID REFERENCES ops.source_match_sets(source_match_set_id)`를 추가하고, `ops.serving_releases.snapshot_id`를 통해 current match set을 찾는 방식이다. 백업 복원처럼 원본 registry가 없을 수 있는 경우에는 `ops.dataset_snapshots.source_set` JSONB 안의 `source_match_set` snapshot을 fallback으로 사용한다.
...
| `invalid` | 참조 group 또는 child file이 missing/quarantined가 되어 사용 불가 |
...
- `source_file_group_id`는 upload session 생성 시 예약한다. DB pending row를 먼저 만드는 B안을 채택하면 `ops.source_file_groups.state='uploading'`으로 생성하고, storage-first A안을 채택하면 session record에만 예약 ID를 둔다.
- `ops.source_files` child row는 기본적으로 `commit` 단계에서 생성한다. 그 전 실패는 upload session 기록에 남기고, B안의 pending group row는 `quarantined` 또는 cleanup 대상으로 전환한다.
...
|----------------------|---------------|----------------|---------------------|-----------|
| `created` | 없음 | 예약 ID 또는 `uploading` pending | 없음 | 세션 생성 |
| `uploading` | 없음 | 예약 ID 또는 `uploading` pending | 없음 | 브라우저 업로드 percent 표시 |
| `uploaded_to_temp` | 없음 | 예약 ID 또는 `uploading` pending | 없음 | temp archive만 존재 |
| `extracting` | 없음 | 예약 ID 또는 `validating` pending | 없음 | temp extract/materialize 진행 |
| `validating_structure` | 없음 | 예약 ID 또는 `validating` pending | 없음 | category validator 진행 |
| `hashing` | 없음 | 예약 ID 또는 `validating` pending | 없음 | archive SHA-256와 group SHA-256 계산 |
| `duplicate_check` | 없음 | 예약 ID 또는 `validating` pending | 없음 | 기존 registry 중복 후보 표시 |
| `storing_to_rustfs` | staging 또는 final 후보 | pending row를 쓰는 B안이면 `state='uploading'`, storage-first A안이면 없음 | 없음 | 최종 commit 방식 결정 필요 |
| `verifying_rustfs_object` | 있음 | pending 또는 없음 | 없음 또는 pending | size/etag/metadata 확인, 필요 시 hash 재계산 |
| `registered` | 있음 | `state='validating'` 또는 `available` 전환 직전 | `state='validating'` 또는 `available` 전환 직전 | members/validations insert |
| `available` | 있음 | `state='available'` | `state='available'` | match set 선택 가능 |
| `failed_upload` | 없음 | 없음 또는 `quarantined` pending | 없음 | 세션 실패 |
| `failed_extract` | 없음 | 없음 또는 `quarantined` pending | 없음 | 세션 실패. 사용자가 archive를 다시 올려야 함 |
| `failed_structure` | 없음 | 없음 또는 `quarantined` | 없음 또는 `quarantined` | 기본은 세션 실패. 사용자가 보존을 선택하면 quarantine row 생성 가능 |
| `failed_hash` | 없음 | 없음 또는 `quarantined` | 없음 | 세션 실패 |
| `failed_rustfs_put` | partial/staging 가능 | 없음 또는 `quarantined` | 없음 또는 `quarantined` | partial cleanup 또는 reconciliation 대상 |
| `failed_rustfs_verify` | 있음 | `missing` 또는 `quarantined` | `missing` 또는 `quarantined` | object 재검증/삭제 필요 |
| `cancelled` | 없음 또는 partial | 없음 또는 `soft_deleted` pending | 없음 | temp/partial cleanup |
...
```text
<prefix>/source-files/<category>/<user_yyyymm>/<source_file_group_id>/<source_file_id>/<original_filename>
```

예:

```text
kor-travel-geo/source-files/roadname_hangul_full/202605/group-3f.../file-7a.../202605_도로명주소 한글_전체분.zip
kor-travel-geo/source-files/electronic_map_full/202604/group-8b.../file-11.../서울특별시.zip
```
...
| 메서드 | 용도 | 주의 |
|--------|------|------|
| `head_object(key)` | commit 후 size/etag/metadata 확인, reconciliation 빠른 scan | SHA-256은 ETag와 같다고 가정하지 않는다. |
| `delete_object(key)` | hard-delete, reconciliation orphan object 삭제 | active match set 참조 여부와 typed confirmation을 먼저 확인한다. |
| `put_file(key, path, sha256=..., metadata=...)` | `x-amz-meta-ktg-*` metadata 저장 | RustFS/S3 호환 차이로 metadata가 사라질 수 있으므로 DB가 정본이다. |
| `copy_object(source_key, dest_key, metadata=...)` 또는 staging commit helper | staging key에서 최종 key로 전환하는 2-phase commit 후보 | 대용량 copy 비용이 커서 최종 선택 필요. |

reconciliation 기본 scan은 object list/head로 얻을 수 있는 key, size, etag, metadata까지만 비교한다. `hash_mismatch` 확정은 대형 object를 다시 읽어야 하므로 기본 자동 scan에서 즉시 수행하지 않고, 사용자가 특정 mismatch 후보를 선택했을 때 on-demand로 SHA-256을 재계산하는 방식을 우선 후보로 둔다.
...
`max_bytes=2147483648`은 현행 `api_max_upload_bytes` 기본값 예시다. 2026-06-14 리뷰 기준으로 SHP 3종은 묶음 ZIP을 만들지 않고 시도별 ZIP을 개별 업로드하므로, 현재 확인된 단일 업로드 파일은 2GiB 한계에 걸리지 않는다. 단일 PUT은 1차 구현 경로로 유지하되, 운영망 불안정·향후 더 큰 원천·재시작 가능한 업로드가 필요해지면 multipart/resumable upload를 별도 운영 강화 작업으로 확장한다.

### 파일 업로드

```text
PUT /v1/admin/source-files/upload-sessions/{upload_session_id}/archive
PUT /v1/admin/source-files/upload-sessions/{upload_session_id}/archive?sido_code=11
```

raw body stream. 응답은 session status다. `single_file`은 query 없이 한 번만 호출하고, `sido_file_set`은 `sido_code`를 명시해 17개 slot을 채운다. 서버가 파일명에서 시도명을 추론할 수 있더라도 최종 slot 배정은 사용자가 선택한 category와 `sido_code`가 우선한다.
...
terminal state는 `available`, `quarantined`, `failed_*`, `cancelled` 중 하나다. 클라이언트는 SSE가 끊기면 `GET /v1/admin/source-files/upload-sessions/{id}` polling으로 fallback한다.
...
`commit`은 다음을 원자적으로 처리해야 한다. 여기서 "원자적"은 사용자에게 보이는 registry 상태가 일관되게 전환된다는 뜻이며, RustFS와 PostgreSQL 사이의 실제 분산 트랜잭션을 뜻하지 않는다.
...
4. RustFS put
5. RustFS head/get 검증. `head_object`로 size/etag/metadata를 먼저 확인하고, 필요할 때만 object를 다시 읽어 SHA-256을 재계산한다.
6. `ops.source_file_groups` insert/update
7. `ops.source_files` insert
8. `ops.source_file_members` insert
9. audit event 기록

완전한 DB transaction과 RustFS put은 분산 트랜잭션이 아니므로, 실패 복구 규칙이 필요하다. 우선 후보는 DB pending row를 먼저 만들고 staging object key에 업로드한 뒤, 검증 성공 시 최종 key를 기록하고 `available`로 전환하는 방식이다. storage-first 흐름을 택하면 RustFS put 성공 후 DB insert 실패 시 reconciliation에서 `object_missing_db`로 잡히게 metadata를 object에 넣는다. DB insert 성공 후 object 확인 실패 시 `source_files.state='missing'` 또는 `quarantined`로 남긴다.

동시성 규칙:

- 같은 `category + user_yyyymm + group_sha256` commit은 PostgreSQL advisory lock으로 직렬화한다. file 단위 중복은 child `sha256 + size_bytes + sido_code`로 warning을 만든다.
...
- orphan staging object는 reconciliation 대상이며, 운영 SLA는 "다음 정기 reconciliation 또는 수동 scan에서 quarantine/delete 후보로 노출"로 둔다.
...
hard-delete, `update_hash_after_verify`, `delete_object`, match set `activate`, `rebuild-db`는 파괴적이거나 고권한 작업이다. 1차 구현에서 별도 로그인/role gate가 없다면 내부망/GeoIP/운영자 콘솔 전제와 typed confirmation을 문서와 UI에 명확히 표시한다. 운영 권장 구현은 admin role gate를 추가해 read-only viewer, source-file manager, rebuild operator, destructive admin 권한을 분리하는 것이다. 어떤 경로를 택하든 `ops.audit_events`에는 actor, request id, 대상 resource id, typed confirmation hash, outcome을 남긴다.
...
UI는 C1~C10을 특별 취급하지 않고 API가 내려주는 case definition 순서대로 탭을 그린다. 새 C11+가 들어와도 가로 스크롤 탭과 sample table이 깨지지 않아야 한다. 1차 구현에서 `CASE_DEFINITIONS` 정적 tuple을 확장하더라도 API 응답 기준으로 렌더링해, 장기적으로 DB registry 기반 case catalog로 바꿀 때 UI 변경을 최소화한다.
...
3. 각 `source_file_group_id`의 child `source_file_id`와 RustFS object 존재 여부 확인
...
- `ops.source_file_groups`, `ops.source_match_set_items.source_file_group_id`, `ops.dataset_snapshots.source_match_set_id`, `ops.consistency_case_samples.case_code` CHECK 완화 포함
...
- RustFS `head_object`, `delete_object`, `put_file(metadata=...)`와 put/head/hash verify
...
| source file group stores group_sha256 and child file metadata | DB metadata 정본 |
| sido_file_set requires 17 sido slots before commit | SHP 3종 multi-file 모델 고정 |
...
| reconciliation detects hash_mismatch through on-demand rehash | 손상 탐지와 비용 통제 |
...
6. RustFS orphan object simulation → reconciliation `object_missing_db`
...
- RustFS object는 기본 무기한 보존이므로 저장소 용량 모니터링이 필요하다. category별 object 수, 총 size, 최근 30일 증가량, `quarantined`/`soft_deleted`/orphan staging size를 admin UI와 운영 metric에 노출한다.
...
- orphan staging object는 무기한 보존 대상이 아니다. reconciliation에서 발견되면 생성 시각, 예상 category, size를 표시하고 수동 삭제 후보로 노출한다.
...
6. RustFS 용량 관리 정책은 기본 무기한 보존을 유지하되, archive tier, soft-deleted object retention, orphan staging cleanup SLA를 별도 ADR로 확정한다.
