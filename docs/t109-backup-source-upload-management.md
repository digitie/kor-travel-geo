...
| 시나리오 누락 집중 리뷰 | 진행 중 upload session 재개 진입점, rebuild 중단 복구, consistency ERROR 승격 gate, match set `invalid` 복귀, 복원 후 `restored_from_backup` stub 생성, 미등록 stored object의 `pending_registration` 분리를 추가 |
...
- 참조 group/file이 `missing`/`quarantined`로 바뀌면 match set은 `invalid`가 된다. 이때 이미 만들어진 serving DB가 즉시 장애가 된다는 뜻은 아니다. UI는 "현재 serving은 유지되지만 원천 archive 결손으로 같은 DB를 재구성할 수 없음" 배지와 누락 object 목록을 표시한다.
- `invalid` 원인이 모두 해소되어 참조 group이 다시 `available`이 되면 자동 active 복귀가 아니라 `revalidatable`로 표시한다. 운영자가 `validate`를 실행해 canonical hash와 coverage가 다시 맞으면 직전 usable state(`validated` 또는 `active`)로 복귀할 수 있다.
- active match set이 `invalid`가 되어도 one-active 슬롯은 유지한다. 슬롯을 비우지 않고 source archive 결손 경고를 단다. 새 match set을 activate하면 기존 invalid active는 `retired`로 전환된다.
...
5. 사용자가 category, `user_yyyymm`, part 정보를 확인한 뒤 registry import 또는 object delete를 선택한다.
6. resolve 직전에는 DB row와 RustFS head를 다시 읽어 동시 register/delete로 인한 오탐을 제거한다.
...
- optional 자료가 있으면 materialize 직후와 validator 실행 직전 archive SHA-256/size를 registry와 대조한다. 불일치/누락은 `skipped`가 아니라 `failed`이며, `validation_inputs.<category>.state='failed'`, `failure_reason='source_integrity_mismatch'`를 남긴다. 새 DB 적재나 snapshot/release 생성은 하지 않고, 해당 group은 `quarantined`, 참조 match set은 `invalid`로 전환한다.
...
PR #131 추가 코멘트를 반영한 뒤, 구현자가 빠뜨리기 쉬운 분기를 다음 표로 고정한다. 결론은 **현재 문서 기준으로 요구 시나리오의 큰 누락은 없으며**, 구현 PR은 아래 행을 테스트·API 응답·UI 상태로 추적 가능하게 만들어야 한다.
...
| Match set 복구 | active match set이 object 손실로 `invalid`가 된 뒤 object를 복구함 | serving 장애가 아니라 재구성 가능성 결손으로 표시한다. group이 모두 `available`이 되면 `revalidatable`로 바꾸고, 명시 validate 후 이전 usable state로 복귀한다. |
| DB 재구성 | RustFS object가 register 이후 교체·손상됐지만 reconciliation이 아직 돌지 않음 | rebuild job이 다운로드/materialize 중 계산한 SHA-256/size를 loader enqueue 직전 재확인한다. mismatch면 child job을 만들지 않고 group `quarantined`, match set `invalid`, audit event를 남긴다. |
...
6. restore 직후에는 rebuild 버튼을 비활성화하고, object availability scan 또는 수동 재연결이 끝나 group이 `available`로 바뀐 뒤 `validate`를 요구한다.
...
- DB row scan
- issue 생성
- `pending_registration`, `source_file_unavailable`, `source_file_group_incomplete`, `delete_failed` issue type
- resolve action
...
- active match set invalid 상태를 serving 장애가 아닌 재구성 불가 경고로 표시
...
| invalid active match set can become revalidatable after object recovery | 원천 결손 복구 전이 |
...
15. active match set의 object를 삭제해 invalid 전환 → 같은 hash object 재연결 → `revalidatable` → validate 후 active 상태 복구
