...
| 백업/복원 | `restored_from_backup` stub에 object를 재연결함 | **두 상태머신을 구분**: group/file은 `missing -> validating -> (validation_state passed/warning) -> available`, match set은 (canonical hash 산출 후) `restored_from_backup -> revalidatable -> (validate) -> validated`. `validation_state='unknown'`인 채 `available`로 바꾸지 않는다. |
