# 처음 보는 사람을 위한 pykraddr 코드 안내서

작성일: 2026-05-10

이 문서는 Google Docs `주소 코드 데이터 구조 및 API 정보`의 내용을 바탕으로,
pykraddr 코드를 처음 읽는 사람이 주소 체계와 코드 구조를 함께 이해할 수 있도록
정리한 입문 문서입니다.
...
pykraddr는 대한민국 주소 데이터를 세 가지 방식으로 다룹니다.
...
- `pykraddr.models.LegalDongRecord`
- `pykraddr.legal_dong.iter_legal_dong_records`
- `pykraddr.postgis.PostGISLegalDongStore`
...
API에서는 `bdMgtSn`으로 오고, pykraddr 저장소에서는
...
- `pykraddr.models.AddressSearchResult.building_management_number`
- `pykraddr.models.RoadNameAddressKoreanRecord.road_address_management_number`
- `pykraddr.store.ROAD_DERIVED_COLUMNS`
...
- `pykraddr.store._road_code_values`
- `road_sigungu_code`
- `road_number`
...
PostGIS 기능까지 확인하려면 WSL2에서 `pykraddr[postgis]`를 설치하고 Docker
...
9. 다른 라이브러리에서 이미 검증된 구현이 있으면 "최소 수정"보다 그 구현을
   pykraddr 코드에 직접 반영하는 쪽을 우선합니다. 단순 위임으로 숨기지 말고,
