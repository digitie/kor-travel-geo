...
| **국가지점번호 문자열 좌표** | **서빙 기능 승격** | v2 forward/reverse 무주소 좌표 | `parse_national_point_number()`가 EPSG:5179 10m cell 중심을 계산한다. 데이터 적재 없이 좌표 활용 가능 | `TL_SPPN_MAKAREA` 밖이라는 이유만으로 좌표 자체를 버리는 정책은 후속 v2에서 해소 |
...
1. T-105 v2 재audit에서 국가지점번호 forward makarea gate 분리, reverse formatter 노출, `point_precision="grid_cell"` 또는 동등한 정밀도 표기를 설계한다.
