...
| `npm run start --hostname ...`가 인자를 못 받음 | npm script 인자는 `--` 뒤에만 하위 명령으로 전달됨 | `npm run start -- --hostname 0.0.0.0 --port 9002` |
...
```bash
source ~/.nvm/nvm.sh
node -e "fetch('http://127.0.0.1:9002/api/runtime-config').then(r=>r.json()).then(j=>console.log(Boolean(j.vworldApiKey), String(j.vworldApiKey || '').length))"
```
