...
- materialize/DB 적재 전에 artifact filesystem free space를 검사한다. 기본 요구량은 120GiB이고
  `KTG_TEST_FULL_LOAD_E2E_LONGRUN_MIN_FREE_GB`로 장비별 override가 가능하다.
- 전체 phase별 wall time, row count, DB size, source month summary를 artifact로 남긴다.
- post-load link evidence는 전국 규모에서 호출 단위 `statement_timeout=0`으로 수집한다.
- serving MV swap 뒤 `ops.dataset_snapshots`와 active `ops.serving_releases`가 기록된다.
- T-183/T-184 착수 전에는 T-177G DB에 live API/UI를 붙여 read-only live UI e2e를 먼저 돌린다.
- 실패 시 재개 가능 phase와 cleanup 절차를 report에 남긴다.

완료 결과(2026-06-18):

- T-188 후속까지 반영한 fresh run `t177g-codex-20260618T043300Z-fresh-t188`가
  `kor_travel_geo_t177g_codex_20260618133300`에서 2:05:08에 통과했다.
- 성공 artifact는
  `artifacts/t177/t177g-codex-20260618T043300Z-fresh-t188/t177g-nationwide-longrun-full-load.json`이다.
- DB size는 35GB이고, `mv_geocode_target`/`mv_geocode_text_search`는 각각 6,419,795행이다.
- post-load smoke, C1~C10 consistency report, ops snapshot/release 기록을 모두 통과했다.
- 같은 DB에 live API/UI를 붙인 Windows Playwright `tests/e2e/live`는 chromium에서 20/20 통과했다.
