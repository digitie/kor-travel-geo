...
Python 패키지의 외부 노출 이름을 더 검색하기 쉽고 직관적인 이름으로 전환한다.
...
- GitHub 저장소 이름 변경은 이 Task에서 자동으로 결정하지 않는다. 필요하면 별도 사용자 결정 또는 ADR로 다룬다.
...
- GitHub 저장소 이름과 Web UI 패키지명까지 맞출지 결정해야 한다.
