...
- 실행 환경: WSL2 ext4 작업본, Docker PostGIS `kor-travel-geo-t027-db-1`, `localhost:15432`
- 기준 데이터: `~/kor-travel-geo-data/juso`
- 기준 검증 명령:

```bash
KTG_PG_DSN=postgresql+psycopg://addr:addr@localhost:15432/kor_travel_geo \
KTG_PG_STATEMENT_TIMEOUT_MS=1800000 \
.venv/bin/ktgctl validate consistency --cases=C2,C4,C6,C7 --scope=t027-data-quality-followup
```
...
- 지도 확인은 디버그 UI와 VWorld/MapLibre wrapper를 사용한다. 범용 wrapper 문제나 표시 한계가 발견되면 `digitie/maplibre-vworld-js`도 수정 대상에 포함하고, 데이터 품질 sample 표시처럼 이 프로젝트에만 의미가 있는 overlay는 `kor-travel-geo-ui`에 둔다.
...
1. C2를 `missing_text`와 `missing_resolve_key`로 나눠 sample CSV를 뽑는다. PR #17에서 `ktgctl validate data-quality-samples`로 구현했다.
...
2026-05-25에 기존 T-027 Docker DB(`kor-travel-geo-t027-db-1`, `localhost:15432`)에서 `--limit 5`로 전체 CSV export를 실행했다.
