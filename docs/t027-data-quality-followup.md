...
성능상으로는 C4 export가 `c4_distance_samples.csv`와 `c4_distance_buckets.csv`에서 같은 nearest distance CTE를 두 번 평가한다. 기능 검증은 통과했지만, 사용자가 요청한 후속 성능 튜닝 PR에서는 이 중복 스캔 제거, 임시 테이블/UNLOGGED staging, case별 materialized intermediate를 10회 이상 trial and error로 비교한다.
