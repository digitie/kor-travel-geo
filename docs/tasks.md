...
- T-044 디버그 UI를 `maplibre-vworld-js`로 완전 포팅 — `kraddr-geo-ui/components/vworld/CoordinateMap.tsx`의 직접 MapLibre wiring을 upstream `digitie/maplibre-vworld-js`의 `VWorldMap` 또는 동등한 재사용 컴포넌트/Hook으로 대체한다. 현재 디버그 UI 계약인 `(lon, lat)` click callback, marker 제어, key 미설정 fallback, transient tile error overlay, redacted logging, `flyTo` 즉시 이동, SSR-safe dynamic wrapper, skeleton/fallback surface를 upstream props/test로 맞춘다. upstream에 부족한 기능·타입·패키징·Next.js 호환 문제가 있으면 `python-kraddr-geo`에서 우회하지 말고 `maplibre-vworld-js` 저장소를 직접 수정하고 별도 PR을 올린 뒤 검증된 SHA로 dependency를 갱신한다. 완료 시 `kraddr-geo-ui` lint/type/test/build와 upstream test/build 결과, 변경 SHA, 남은 차이를 문서화한다.
