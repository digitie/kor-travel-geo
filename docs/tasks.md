...
## 대기 (우선순위 순)
- T-043 PR #23~최신 PR 리뷰 코멘트 일괄 audit/fixup — T-036 이후 누적된 PR #23부터 T-043 실행 시점의 최신 merged/open PR까지 GitHub 리뷰 표면을 다시 읽는다. `gh pr view <번호> --json comments,reviews,latestReviews`만 보지 말고 `gh api repos/digitie/python-kraddr-geo/pulls/<번호>/comments`와 GraphQL `reviewThreads`까지 확인한다. 각 PR별로 conversation comment, formal review body, inline review thread, unresolved/resolved 상태를 표로 정리하고, 반영 가능한 내용은 코드/문서 수정 PR로 반영한다. 이미 merge된 PR의 코멘트는 후속 fixup PR에 이관하고, open PR이 있으면 해당 PR에 직접 반영한다. 완료 산출물은 `docs/postmerge-review-fixups-pr23-latest.md`에 남기고, 반영 후 원 PR 또는 후속 PR에 요약 코멘트를 작성한다.
- T-049 운영 메타데이터·감사·릴리스 스키마 구현 — 유지보수와 운영 복구를 위해 `ops` 스키마를 추가하고 `ops.audit_events`, `ops.dataset_snapshots`, `ops.serving_releases`, `ops.artifacts`, `ops.maintenance_windows`, `ops.table_stats_snapshots`를 구현한다. `load_jobs` 상태 전환, source set 확정, consistency/performance report 생성, backup/restore, MV swap, destructive maintenance 작업은 redacted audit event와 snapshot/release/artifact에 연결한다. destructive restore, schema migration, full reset은 active maintenance window와 typed confirmation 없이는 실패해야 한다. `/v1/admin/ops/*` API와 `/admin/ops` 또는 기존 관리 화면 탭을 추가하고, secret/DSN/token/address 원문이 감사 테이블에 평문 저장되지 않는 테스트를 포함한다. 설계 문서: ADR-033, `docs/t049-ops-metadata-schema.md`
- T-045 원천 자료 기준월 선택과 대용량 업로드/적재 UX — 원천별 업데이트 시점이 달라 `juso=202603`, `locsum/navi/shp=202604`, `roadaddr_entrance=202605`처럼 기준월이 섞일 수 있음을 전제로 `source_set` 계획 객체를 도입한다. CLI는 기준월 불일치 발견 시 사람이 의도한 혼합 적재인지 확인하고, 비대화형 실행은 `--allow-mixed-yyyymm`와 명시 확인 token 없이는 실패한다. API/라이브러리는 디렉터리를 읽어 후보를 매칭하는 함수와 각 원천 기준월/경로를 입력해 적재 계획을 만드는 함수를 분리한다. UI는 다중 파일 선택과 DND 업로드를 지원하고, 모든 파일 저장 완료 후 기준월 mismatch 팝업 확인을 거쳐 적재를 시작한다. 업로드 진행률과 적재 진행률은 별도 퍼센트로 표시하고, 업로드/적재 모두 중간 취소가 가능해야 한다. 설계 문서: ADR-029, `docs/t045-source-set-load-ux.md`
