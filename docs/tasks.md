...
- ✅ **T-297** — 디스크 위험 사후 조치를 완료 처리했다. 남은 재발 방지 검토는 낮은 우선순위
  후속으로 보존한다.
- 🔄 **T-299** — blue 색상톤을 유지한 상태에서 색각 구분성·상태 의미 전달을 다시 검토 중이다.
- 🔄 **T-300** — 공통 포커스 링의 비텍스트 대비·키보드 이동·메뉴/테이블 포커스를 다시 검토 중이다.
- 🔄 **T-303** — 최신 `kor-travel-map` 기준 메인 콘텐츠의 전체 look and feel 동기화 작업 중이다.
