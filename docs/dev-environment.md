...
`.env*`, `.claude/`, `.codegraph/`는 ignore 대상이어야 한다. API 로컬 포트는 `9001`이므로 `KRADDR_GEO_API_INTERNAL_URL` 예시는 `http://localhost:9001`로 맞춘다.
...
| `kraddr-geo-ui` | `9002` | `9002` | Docker는 `-p 9002:9002`, local dev는 `npm run dev -- --port 9002` |
...
- **npm 서버 파라미터 전달**: Next.js 서버 옵션은 `npm run start -- --hostname 0.0.0.0 --port 9002`처럼 `--` 뒤에 둔다. `npm run start --hostname ...` 형태는 쓰지 않는다.
