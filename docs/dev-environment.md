...
PostgreSQL 검증은 이 저장소가 직접 DB를 띄우지 않고, `KRADDR_GEO_PG_DSN`이 가리키는 이미 동작 중인 PostgreSQL/PostGIS에 접속해 수행한다. RustFS도 `KRADDR_GEO_RUSTFS_*` 설정으로 이미 준비된 bucket에 접속한다. 이 저장소에는 DB/RustFS Docker 구동·정지·재시작 절차를 두지 않는다.
...
`.env*`, `.claude/`, `.codegraph/`는 ignore 대상이어야 한다. API 로컬 포트는 `12201`이므로 `KRADDR_GEO_API_INTERNAL_URL` 예시는 `http://localhost:12201`로 맞춘다.
...
이 저장소의 PC/WSL 개발 환경은 다음 host 포트를 공식값으로 사용한다. PostgreSQL은 표준 포트 `5432`를 사용하고, RustFS API는 `12101`, 이 저장소의 API/UI는 `12201`/`12205`를 사용한다.

| 표면 | 공식 host 포트 | 내부 포트 | 비고 |
|------|----------------|-----------|------|
| FastAPI 백엔드 | `12201` | `12201` | `uvicorn kraddr.geo.api.app:app --host 127.0.0.1 --port 12201` |
| `kraddr-geo-ui` | `12205` | `12205` | Docker는 `-p 12205:12205`, local dev는 `npm run dev -- --port 12205` |
...
- **npm 서버 파라미터 전달**: Next.js 서버 옵션은 `npm run start -- --hostname 0.0.0.0 --port 12205`처럼 `--` 뒤에 둔다. `npm run start --hostname ...` 형태는 쓰지 않는다.
