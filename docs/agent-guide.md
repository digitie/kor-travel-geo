...
작업 디렉터리는 에이전트별 NTFS 고정 worktree를 우선한다. ChatGPT Codex는 `/mnt/f/dev/python-kraddr-geo-codex`, Claude Code는 `/mnt/f/dev/python-kraddr-geo-claude`, Google Antigravity 2.0은 `/mnt/f/dev/python-kraddr-geo-antigravity`를 사용한다. worktree가 없다면 `docs/dev-environment.md` §1.1 절차로 생성하고, `.codegraph/`가 없다면 해당 worktree에서 `codegraph init -i`를 최초 1회 실행한다. NTFS `/mnt` 경로에서는 CodeGraph live watch가 비활성화될 수 있으므로 새 작업 branch를 만든 직후 `codegraph sync`와 `codegraph status`로 인덱스를 맞춘다. 테스트와 장기 실행은 worktree를 WSL ext4 테스트 미러로 복사한 뒤 수행하며, 미러에서는 commit/push하지 않는다. 미러 셋업·검증 루프·반복 함정(Windows npm shim, Windows Temp) 해결을 단계별로 따라하려면 `docs/agent-workflow.md` 런북을 보고, 미러 셸에서는 `source scripts/agent_env.sh`를 먼저 실행한다.
