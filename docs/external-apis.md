...
본 문서는 `kor-travel-geo`이 호출하거나 디버그 UI에서 사용하는 외부 API(vworld, juso, epost)의 발급 절차, 환경변수 매핑, 호출 예시, 정책을 한 자리에 모은다. 첨부 사양서 §13.3을 기준으로 정리했다.

## 한눈에

| 서비스 | 발급처 | 용도 | 환경변수 | 키 노출 위치 |
|--------|--------|------|----------|--------------|
| vworld OpenAPI | vworld.kr | 지오코딩 폴백, 통합 검색 | `KTG_VWORLD_API_KEY` | 서버측 |
| vworld WMTS | vworld.kr | 프론트엔드 디버그 지도(MapLibre) | `KTG_VWORLD_API_KEY`, `NEXT_PUBLIC_VWORLD_API_KEY` | 브라우저 (도메인/IP 제한이 보안) |
| juso 검색 | business.juso.go.kr | 도로명/지번 주소 검색 폴백 | `KTG_JUSO_API_KEY` | 서버측 |
| juso 좌표 | business.juso.go.kr (별도 신청 가능) | 주소 → 좌표 변환 폴백 | `KTG_JUSO_COORD_API_KEY` (없으면 검색 키 재사용) | 서버측 |
| epost 우편번호 다운로드 (데이터셋 `15000302`) | data.go.kr (공공데이터포털) | 사서함·다량배달처 ZIP 분기 1회 적재(ADR-009) | `KTG_EPOST_API_KEY` | 서버측 (로더 cron) |
...
async with httpx.AsyncClient(timeout=5.0) as cx:
    ...
    }, headers={"Referer": "https://kor-travel-geo.your-domain.local"})
    data = r.json()
...
KTG_VWORLD_API_KEY=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
KTG_JUSO_API_KEY=devU01TX0FVVEgyMDIxMDExNTAxNTAxMDExMTAzMDM=
KTG_JUSO_COORD_API_KEY=                # 비워두면 위 키 사용
KTG_EPOST_API_KEY=urlDecoded+ServiceKey+Value==
```
...
- **구현 위치**: `src/kortravelgeo/infra/external_api.py`. `AsyncAddressClient.geocode(..., fallback="api")`에서 내부 v1 호환 geocode 결과가 로컬 `NOT_FOUND`일 때만 호출하고, 공개 응답은 후보 schema로 투영한다. core 계층은 HTTP와 API key를 전혀 알지 않는다.
