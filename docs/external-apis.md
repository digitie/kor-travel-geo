...
- **키 소스**: 로컬 개발에서는 Python API `.env`의 `KRADDR_GEO_VWORLD_API_KEY`를 UI 지도에도 재사용한다. 운영에서 백엔드 fallback 키와 브라우저 WMTS 키를 분리해야 하면 `KRADDR_GEO_VWORLD_API_KEY`를 서버 프로세스 환경에서 빼고 `NEXT_PUBLIC_VWORLD_API_KEY`만 제공한다. 로컬 개발용 키에는 `localhost:12205`, `127.0.0.1:12205`, WSL e2e에 쓰는 내부 IP와 포트 조합을 등록한다.
...
```bash
# .env (백엔드)
KRADDR_GEO_PG_DSN=postgresql+psycopg://addr:CHANGEME@localhost:5432/kraddr_geo
