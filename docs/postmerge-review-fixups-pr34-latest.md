...
| #36 | merged | T-046 backup/restore progress, zstd, callback 보안, empty DB, `ops.artifacts` 수렴 | T-046 구현에서 path allowlist, `pg_dump -Fd`, zstd, manifest/checksum, 새 DB guard, download token, `ops.artifacts` 연결 반영. callback HMAC/retry/replay 판별은 T-050 2차에서 반영 |
...
- `openapi.json`, `kor-travel-geo-ui/types/api.gen.ts`, `kor-travel-geo-ui/lib/api.ts` 갱신
...
- `kor-travel-geo-ui/package.json`, `package-lock.json`의 `maplibre-vworld` SHA를 `7947b2e...`로 갱신
- lockfile `resolved`는 CI SSH key 의존을 피하기 위해 `git+https` 유지
- `docs/frontend-package.md`, `docs/external-apis.md`, ADR-020/028/032, `docs/resume.md`, `docs/tasks.md`, `kor-travel-geo-ui/README.md`의 현재 SHA 갱신
...
- Playwright는 사용자 지시에 따라 Windows Node/브라우저에서만 실행한다고 `docs/dev-environment.md`와 `kor-travel-geo-ui/README.md`에 명시
...
| PR #36 M1/M3 | backup/restore sub-progress 고도화, callback HMAC/retry/replay protection | T-050 운영 hardening. callback HMAC/retry/replay 판별은 T-050 2차 완료, file/archive size 기반 sub-progress는 T-050 3차 완료 |
...
- `source_set` 타입 변경은 `scripts/export_openapi.py`와 `kor-travel-geo-ui npm run gen:types`를 함께 수행한다.
