...
T-041 최초 결론에서는 두 레이어 모두 overlay/분석 후보로 보류했다. 하지만 `TL_SPPN_MAKAREA`는 단순 화면 overlay가 아니라 **국가지점번호 표기 의무지역 polygon**이다. 주소가 없는 산악·해안·하천·도서 등 비거주지역에서 geocode/reverse geocode를 보조할 수 있는 데이터이므로, 후속 구현에서는 `tl_sppn_makarea` 별도 테이블로 적재한다. 단, 이 레이어는 개별 국가지점번호판 point 목록이 아니라 "지점번호를 표기해야 하는 구역"의 면 데이터다. 따라서 현행 `mv_geocode_target`의 도로명/지번 주소 1행 계약에 섞지 않고, 국가지점번호 전용 보조 후보와 `x_extension` 확장으로 노출한다.
...
좌표가 여러 polygon에 포함되면 더 작은 면적의 polygon을 우선한다. `ST_Contains` 대신 `ST_Covers`를 사용해 경계선 위의 좌표도 놓치지 않는다. 결과는 vworld 호환 주소 필드에 억지로 끼워 넣지 않고, `x_extension.sppn_makarea` 또는 별도 `type='sppn_area'` 후보로 표현한다.
...
이 문서 보강은 코드 구현을 포함하지 않는다. 후속 구현 태스크는 다음으로 분리한다.

1. `TL_SPPN_MAKAREA` loader와 `tl_sppn_makarea` DDL 추가.
2. reverse geocode에서 `sppn_area` 보조 후보를 조회하되, 기존 도로명/지번 후보와 응답 계약이 섞이지 않도록 `x_extension` 또는 별도 후보 type으로 노출.
3. 국가지점번호 문자열 parser/generator를 별도 pure-core 모듈로 설계. `TL_SPPN_MAKAREA`는 parser가 계산한 좌표의 지역 검증과 문맥 보강에 사용.
4. 디버그 UI 지도 overlay에서 `TL_SPPN_MAKAREA` polygon과 reverse 결과를 함께 표시.
