...
T-044에서는 이 보류분을 해소한다. 목표는 `CoordinateMap.tsx`의 직접 MapLibre wiring을 upstream `maplibre-vworld-js`의 `VWorldMap` 또는 동등한 Hook/component로 완전히 포팅하는 것이다. 부족한 upstream 기능이 있으면 `python-kraddr-geo`에 workaround를 쌓지 않고 `digitie/maplibre-vworld-js`를 직접 수정한다.
...
- `maplibre-vworld-js`가 click callback, marker 제어, `flyToOptions`, tile error hook/redaction, key 미설정 fallback, SSR-safe 사용법을 public API와 test로 제공한다.
