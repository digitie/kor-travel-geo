...
| 출처 | 항목 | 처리 |
|------|------|------|
| PR #51 M1, PR #52 M2 | `pg_stat_statements` 비활성 | 후속 T-047 run에서 PostgreSQL `shared_preload_libraries='pg_stat_statements'`, run 전후 `pg_stat_statements_reset()`/snapshot artifact를 추가한다. |
...
| PR #52 M4 | client wall time이 pool wait/DB execution 미분리 | 후속 run에서 connection-preacquired 측정 또는 `pg_stat_statements`/EXPLAIN aggregate로 분리한다. |
| PR #52 M5 | `iterations=1` sample 부족 | 후보 확정 run은 `--iterations 3` 이상으로 수행하도록 T-047 문서에 명시했다. |

## 다음 실행 순서

1. T-047 observability run: `pg_stat_statements` 활성화, `standard --iterations 3`, pool wait/DB execution 분리.
2. T-047 operational impact run: T-047 인덱스 3개 포함 상태에서 MV refresh/swap, backup archive 크기, 디스크 여유 envelope 측정.
3. T-047 stress run: 10,000건 이상 corpus로 c1/c4/c16/c64 측정.
4. Q3 fuzzy 후보 축소: T-057 region hint 또는 `mv_geocode_text_search` 후보와 함께 비교.
5. T-052 또는 SQL 재사용 확대 시점에 SQL 상수 public module 추출.

## 검증

본 작업은 문서 반영이다. 별도 코드 실행 경로는 바뀌지 않는다.
