...
| `도로명주소 건물 도형` | 중간 | 높음 | 현 `tl_spbd_buld_polygon` 대체 금지. 별도 테이블로 출입구/연결선 검증 후 후보 승격 |
| `건물군 내 상세주소 동 도형` | 상세주소 기능에는 높음, 일반 주소에는 낮음 | 높음 | 상세주소 동/출입구 별도 기능 후보. 기본 geocode에는 섞지 않음 |
| `상세주소DB` | 상세주소 기능에는 중간 | 높음 | 상세주소 parser/검증 원천. 기본 주소 geocode 개선용 아님 |
...
즉, 일반 도로명주소 geocode/reverse 정확도에 바로 도움이 되는 후보는 `도로명주소 출입구 정보`와 `도로명주소 건물 도형`의 출입구/연결선 계열이다. 국가지점번호 도형/중심점은 현재 10m 계산식보다 정밀한 좌표를 주는 데이터가 아니므로, 직접 정확도 개선보다는 검증과 coarser grid 지원에 적합하다.
