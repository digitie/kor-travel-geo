...
- 🔄 **T-303 — 최신 `kor-travel-map` 기준 메인 콘텐츠 전체 look and feel 동기화 진행 중** —
  페이지 제목·본문 스케일뿐 아니라 패널/카드, 폼 컨트롤, 테이블, 상태 요약, 메뉴, 반응형
  여백과 포커스 표시를 함께 대조해 draft PR #543에 반영했다. 기존 Workbench 구조와 blue
  색상톤은 유지했으며, 운영 UI live e2e와 머지가 남았다.
- ✅ **T-297 — 완료 처리** — 즉시 디스크 위험 해소 및 데이터 무결성 확인 이력을 완료 목록으로
  이동했다. 재발 방지 검토만 낮은 우선순위 후속으로 남겼다.
- 🔄 **T-299/T-300 — 진행 중** — 색각 구분성·상태 의미 전달과 포커스 링 대비·키보드 경로를
  전체 공용 primitive 및 메뉴 검토에 포함해 보완했고, 운영 검증 결과를 기다린다.
...
- ✅ **T-301 — admin UI brand blue를 사용자 지정 레퍼런스 팔레트로 재적용 (2026-08-27,
  ...
  0.104) 대신, 사용자가 준 레퍼런스 팔레트 HTML의 "Blue"(Tailwind blue-600/700/100:
  `#2563EB`/`#1D4ED8`/`#DBEAFE`)를 OKLCH 역변환으로 정확히 재현해 brand/accent/tint
  토큰을 교체(hue 262.9, chroma 0.215로 상승). paper 배경 계열은 동일 hue-rotation
  아키텍처로 새 hue에 맞춰 갱신. 적대적 리뷰 2건 모두 실이슈 없음 — 특히 리뷰어 B가
  T-299(brand-info hue 근접) 우려를 더 엄밀한 방법(Brettel 1997 tritanopia sim +
  OKLab distance)으로 재검증해 chroma가 hue-angle보다 지배적이라 실제로는 더 잘
  구분됨을 확인(OKLab distance 61%↑, tritanopia ΔE 2.5배↑) — T-299는 그대로
  accepted-tradeoff 유지, 재조사 불필요. 새 accent chroma(0.215)가 이제 danger
...
- ✅ **T-298 — admin UI 컬러톤 teal → blue 전환 (2026-08-27, PR #540, 머지·n150 배포
  ...
  T-301이 정확한 값을 사용자 레퍼런스로 재조정했으므로 현재 실효 색상은 T-301 참조.
  상세는 `tasks-done.md`.
...
- ✅ **이슈 #525 — C11~C17 가드 + src/ 헬퍼 blind spot (2026-08-25, PR #526, by claude)** —
  ...
  DSN을 거부하면 세션이 실패하는 **skip budget**을 넣었다(#523에서 T-246이 조용히 사라진 걸
  잡았을 장치). C14는 DB 테스트가 아니라 범위에서 제외.
...
- ✅ Linux-only 개발환경 정책 문서화 — 사용자 지시에 따라 ADR-065를 추가하고 ADR-041을 superseded 처리했다.
  ...
  Windows fallback을 기록한다. `AGENTS.md`, `SKILL.md`, `README.md`, `docs/dev-environment.md`,
  `docs/runbooks/agent-workflow.md`, `docs/runbooks/agent-failure-patterns.md`,
  `docs/codegraph-worktree.md`, `docs/live-e2e.md`, `docs/geocoding-readiness.md`를 같은 기준으로 갱신했다.
...
- ✅ T-177A 파일 기반 full-load e2e 계획/Task 등록 완료 — T-073 shell script 재실행에 맞추지
...
- ✅ T-177F post-load serving/smoke/consistency fast-sample e2e 완료 — opt-in 상태에서 실제
  정본 텍스트 snapshot, 위치정보요약DB, selected 전자지도 SHP, roadaddr entrance, SPPN makarea를
  한 scratch DB에 적재한 뒤 `resolve_text_geometry_links()`와 `rebuild_mv()`로
  `mv_geocode_target`/`mv_geocode_text_search`를 구축한다. `region_radius_parts`는 T-177D SHP
  helper가 갱신한 serving object로 함께 검증한다. 테스트는 cache-off
  geocode/reverse/search/zipcode smoke, 위치정보요약DB 링크 기반 smoke sample, C1~C10
  consistency report 생성, `t177f-postload-serving-smoke.json` artifact를 검증한다.
  fast-sample C1~C10 severity는 acceptance gate가 아니라 SQL 실행/샘플 산출 smoke로 기록한다.
...
- ✅ T-181 전국 long-run 링크 증거 집계 timeout 해소 — T-177G 실제 전국 원천 적재와
  `refresh_mv(strategy="swap")` 완료 뒤 `collect_t177f_link_evidence()`의 `EXISTS` 기반
...
- ✅ T-153 최종 안정화 acceptance 완료 — Agent A 성능·정확도 트랙(T-138, T-140~T-146, T-154~T-176)과 Agent B Admin UI·백업/복원 트랙(T-220~T-263)을 `docs/t153-final-stabilization-acceptance.md`로 묶어 수락했다. Golden corpus fixture 25/25, C1~C17 상태, SQL/REST c64 budget, Admin UI Playwright, 백업/복원 round-trip·fault injection·hot-swap, React Doctor, OpenAPI/typegen drift를 함께 정리했다. 새 release blocker는 없다.
  T-119/C11 active promotion은 no-go 종료했고 T-139 DB 구조 변경 실험은 no-action 종료했다.
  남은 것은 실제 60분 live soak, T-063 N150/Odroid 실측, T-219 M4 전역 검증 핸들러 후속, T-105 audit 이후 ADR-060 반영 backlog다.
- ✅ T-127 optional source 구조 validator 강화 완료 — `detail_address_db_full`, `national_point_grid_shape`, `national_point_grid_center`, `civil_service_institution_map`, `address_db_full`, `building_db_full`의 single-file archive member naming, 필수 TXT/SHP layer, `.shp/.shx/.dbf` sidecar, 기준월 혼재 warning profile을 추가했다. legacy ZIP의 CP949 member name을 복원해 주소DB·민원행정기관 같은 한글 파일명 원천도 검증한다. 실제 보존 원천 smoke는 `national_point_grid_shape`만 `.prj` 없음 `warning`, 나머지 5개는 `passed`다. 상세는 `docs/t127-optional-source-validator.md`.
...
- ✅ T-134 C11 v1/v2 좌표 출처 노출 계약 완료 — `docs/t134-c11-coordinate-source-contract.md`와 ADR-055를 추가해 `pt_source`는 coarse enum `entrance`/`centroid`만 유지하고, 세부 원천은 `coord_source_detail`로 분리하기로 확정했다. v1은 최상위/result field를 추가하지 않고 필요 시 `response.x_extension.pt_source`/`coord_source_detail`만 사용한다. v1 reverse는 후보별 detail을 새로 노출하지 않으며, v2는 `CandidateV2.point_precision`과 `metadata.pt_source`/`metadata.coord_source_detail`을 사용한다. 현재 코드에는 해당 DTO field가 없으므로 이번 PR에서는 OpenAPI/typegen/code를 바꾸지 않고, 구현 테스트 계획만 문서화했다. `docs/architecture/data-model.md`의 기존 `x_extension.pt_source` 노출 설명도 현재 코드와 ADR-055 기준으로 정정했다.
...
- ✅ T-130 C11 C4/C6/C7 회귀 원인 분석 완료 — C4 over500 68건은 candidate regression 52건, shared 16건으로 분리됐다. C6 candidate error 3,635건 중 회귀 2,834건은 대부분 baseline 출입구 결측 row(2,827건)가 C11 후보로 새로 평가된 결과였고, C7도 회귀 3,087건 중 3,077건이 같은 구조다. Artifact는 `F:\dev\geodata\t130-c11-regression-root-cause\20260616-r1\`, 상세는 `docs/t130-c11-regression-root-cause.md`. GitHub 스캔/merge 중 확인한 Windows Python cp949 디코딩과 `gh pr merge` repo 생략 실패 패턴은 `docs/runbooks/agent-failure-patterns.md`에 기록했다. T-131 simulation을 위해 `_ktg_t125_*` 작업 테이블은 남겨 두었다.
...
- ✅ PR #187 리뷰 후속 반영 — post-merge 상세 리뷰에서 blocking은 없었고 Low 2건을 처리했다. `source_member_scan`의 layer token 추출은 dot 전용에서 token boundary 기반으로 넓혀 underscore/hyphen/space 구분 파일명도 인식한다. 구분자 없이 완전히 붙은 vendor 파일명은 구조 검증 실패로 드러나게 full-stem fallback을 유지한다. optional single-file category 상세 validator 강화는 T-127에서 완료했다.
...
- ✅ 이번 세션 실행 실수 복기 문서화 — `docs/runbooks/agent-failure-patterns.md`와 `docs/runbooks/agent-workflow.md`에 `gh --repo` 사용, npm script 인자 `--` 전달, WSL Linux Node 초기화, WSL UI 서버 + Windows Playwright 실행, server PID 종료, CodeGraph sync/status 순서, generated `next-env.d.ts` 복구, 같은 실패 명령 반복 금지 규칙을 추가했다.
...
- 최신 디버그 지도 의존성 기준은 `docs/architecture/frontend-package.md`와 **ADR-063**에 있다(ADR-020/028/032는 의존성 선택만 ADR-063으로 넘긴다). 현재 `kor-travel-geo-ui` dependency는 GitHub `digitie/maplibre-vworld-react` tarball SHA `a7cb0f8f41ec00b44b1d106664506730b87033bd`이고, npm registry에는 아직 공개 package가 없어 `.../archive/<sha>.tar.gz` HTTPS tarball로 설치한다(git+https 아님). T-277(#362)에서 전환했다.
...
- 디버그 UI의 실제 브라우저 회귀는 n150 Linux 환경에서 Playwright를 먼저 실행한다. PR 완료 전 Chrome 기준 `--project chromium`과 Firefox 기준 `--project firefox`를 모두 실행한다. n150에서 실행할 수 없는 경우에만 Windows Playwright를 fallback으로 사용하고, fallback 사유와 명령을 작업 기록에 남긴다.
...
- **Linux-only 개발 기준**: Git source of truth는 Linux `git`이 읽는 고정 worktree이고 테스트 실행은 ext4 미러다. ext4 미러에서 commit/push하지 말고, 필요한 변경은 고정 worktree에 반영한다.
- **Git/CodeGraph는 Linux 기준**: `.git`/`gitdir` 포인터가 `F:/...` 같은 Windows 경로를 가리키면 Linux에서 `git worktree repair <worktree>`를 실행하거나 worktree를 재생성해 `/mnt/f/...` 또는 ext4 경로로 맞춘다.
...
- **VWorld debug map**: runtime config는 Python API `.env`의 `KTG_VWORLD_API_KEY`를 우선 사용한다. `maplibre-vworld-react`는 현재 GitHub HTTPS tarball `https://github.com/digitie/maplibre-vworld-react/archive/a7cb0f8f41ec00b44b1d106664506730b87033bd.tar.gz`로 고정되어 있고(git+https/SSH 아님 — raw monorepo source tarball), `CoordinateMap`은 upstream `VWorldMapView`(=`VWorldMap`)/`Marker`/hook 기반 domain wrapper다. SHA를 바꾸면 새 tarball SHA로 dependency spec과 lockfile을 갱신하고(소비 import 그래프가 새로 닿는 런타임 의존성은 UI `package.json`에 직접 선언해야 한다 — tarball은 inner workspace deps를 설치하지 않는다), Linux Node/npm 또는 Docker Node로 `npm ci`/`lint`/`type-check`/`test`/Next.js build를 다시 확인한다. GitHub archive tarball은 바이트 안정성이 보장되지 않아 같은 SHA라도 후일 `npm ci`가 `EINTEGRITY`로 실패할 수 있다 — 이때는 lockfile의 해당 `integrity`를 지우고 `npm install`로 재생성한다(상세 ADR-063). Playwright e2e는 n150 Linux 환경에서 먼저 실행하고, 불가할 때만 Windows fallback을 사용한다.
