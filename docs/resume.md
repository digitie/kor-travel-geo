...
- 🟡 T-050 운영 hardening 3차 — backup/restore file/archive size 기반 sub-progress를 진행 중이다. dump/archive/checksum/extract 구간에서 파일 크기 sampler를 사용해 기존 `load_jobs.progress/current_stage/log_tail`에 byte 기반 보조 진행률을 남긴다. 상세: `docs/t050-ops-hardening.md`

## 다음 한 작업 (1시간 이내 분량)

다음 작업은 T-050 3차 backup/restore file/archive size 기반 sub-progress PR을 검증·머지한 뒤, T-050 4차 full-load/MV/restore 완료 hook의 `ops.dataset_snapshots`/`ops.serving_releases` 자동 생성을 진행하는 것이다. 이후 `ops.table_stats_snapshots` 주기 capture, destructive confirmation flow 통합, 실제 PostgreSQL constraint integration test를 순서대로 좁힌다.
...
- T-050은 PR #34~#47 리뷰 audit에서 남긴 운영 hardening 묶음이다. 1차 upload set cleanup TTL/참조 lock/grace period와 2차 callback HMAC/retry/replay protection은 완료했고, 3차 size 기반 backup/restore sub-progress는 PR 검증 중이다. 남은 항목은 snapshot/release 자동 생성 hook, table stats cron, destructive confirmation flow, 실제 PostgreSQL constraint integration test다.
