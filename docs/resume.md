...
T-037 PR을 열어 약 20분 리뷰 코멘트를 기다린 뒤, 코멘트가 있으면 최대한 반영하고 없으면 main에 merge한다. 그 다음 작업은 T-027 최종 실 데이터 클린 적재 검증이다. T-027은 Docker DB를 삭제하고 처음부터 다시 적재하므로 실행 전 `docs/t027-fullload-plan.md`의 phase timer와 중단·재개 정책을 다시 확인한다.
