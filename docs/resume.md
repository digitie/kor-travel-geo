...
- [/ ] concierge #127 공개 origin allowlist를 `kor-travel-geo-ui`에도 반영했다.
  ...
  `서울특별시 중구 세종대로 110` v1 geocode smoke도 `status=OK`로 응답했다. 다만 n150 API
  컨테이너의 `/data/juso` 원천 파일 mount는 비어 있어 docker-manager source-file precheck는
  별도 조치 전 계속 실패할 수 있다. 남은 작업은 문서 커밋, PR 생성, 필요 시 n150 배포와 live
  e2e 재실행이다.
