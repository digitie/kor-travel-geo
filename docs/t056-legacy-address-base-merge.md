# T-056: `python-legacy-address-base` Address 부분 정리 + 외부 라이브러리 삭제 대비

## 상태

- 상태: 구현 완료
- 대상 브랜치: `codex/t056-legacy-address-base-address-merge`
- 관련 ADR: ADR-035
- 사용자 RFC: 2026-05-27 — "`python-legacy-address-base`의 Address 파트 병합, Address 코드에 대한 조합/분리 작업만 가져올 것. `python-legacy-address-base` 라이브러리는 삭제할 예정임."
...
| 항목 | 결과 |
|------|------|
| 경로 | `/home/digitie/dev/python-legacy-address-base` |
| Git 상태 | `.git` 없음. `git rev-parse HEAD`는 `fatal: not a git repository` |
| 패키지명/버전 | `python-legacy-address-base` / `0.1.5` |
| 라이선스 | `GPL-3.0-or-later` |
| Address 구현 파일 | `src/legacy/address-base/addresses.py` |
...
사전 문서에서 예상했던 `legacy.address_base.address.parser`, `legacy.address_base.address.composer`, `legacy.address_base.address.types`, `legacy.address_base.address.normalize` package는 존재하지 않았다. 실제 후보는 다음 단일 모듈이었다.

- `src/legacy/address-base/addresses.py`
...
도로명/지번 문자열을 보수적으로 분해하는 로직은 이미 본 저장소의 `src/kortravelgeo/core/normalize.py`에 있고, 원본 패키지에는 별도 문자열 parser가 없었다. 따라서 `core.normalize.parse_address()`는 그대로 유지한다.
...
- 향후 `python-legacy-address-base`가 archive되면 `docs/journal.md`에 archive URL과 마지막 확인 가능한 release/tag 또는 파일 SHA256을 추가한다.

## 검증

이번 PR의 직접 검증 기준:

- `rg "from kortravel\\.base|import kortravel\\.base|legacy-address-base" pyproject.toml src tests scripts`에서 dependency/import 0건. 문서상의 task 설명은 예외적으로 이 파일과 관련 ADR에만 남긴다.
...
- Juso 검색 결과에 `admCd`/`rnMgtSn` 등 좌표 API 필수 코드가 없으면 좌표 API를 호출하지 않고 graceful `None`으로 끝난다.
- 전체 검증은 `pytest -q`, `ruff check .`, `mypy src/kortravelgeo`, `lint-imports`로 수행한다.
...
- `python-legacy-address-base` 자체 archive/read-only 전환은 별도 저장소 운영 작업이다. 본 저장소에서는 더 이상 dependency나 import를 추가하지 않는다.
