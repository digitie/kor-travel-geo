...
`pt_source`는 DB/Repository 내부 coarse 좌표 분류다. 현재 geocode 응답은 이 값을 직접 field로 직렬화하지 않고, `pt_source = 'centroid'` 결과의 신뢰도(`confidence`)를 낮추는 데 사용한다. T-134/ADR-055 이후 좌표 출처를 노출해야 하면 v1은 `response.x_extension.pt_source`/`coord_source_detail`, v2는 `CandidateV2.metadata.pt_source`/`coord_source_detail`을 사용한다. `pt_source` enum은 `entrance`/`centroid`만 유지하고 세부 원천명으로 확장하지 않는다.
