...
T-049부터 운영 메타데이터는 `ops` 스키마에 둔다. `public`은 주소 원천·serving 객체를 유지하고, `x_extension`은 PostGIS 보조 extension 격리 용도로 유지한다. `ops` 객체는 감사, 데이터셋 snapshot, serving release, artifact registry, maintenance window, table stats snapshot, `pg_stat_statements` snapshot을 담당한다. 실제 DDL은 `sql/ddl/001_schema.sql`, 최초 migration은 `alembic/versions/0006_t049_ops_metadata_schema.py`, T-157 `pg_stat_statements` snapshot migration은 `alembic/versions/0019_t157_pg_stat_snapshots.py`에 있다.
