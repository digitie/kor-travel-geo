...
- inline review comment: REST `GET /repos/digitie/python-kraddr-geo/pulls/{number}/comments`
...
- `kraddr-geo-ui/lib/vworld.ts`의 `redactVWorldTileUrl` alias에 수명 정책 주석을 추가했다. T-044에서 `redactVWorldUrl`로 호출자를 옮긴 뒤 제거한다.
- `kraddr-geo-ui/tests/unit/vworld.test.ts`에 API key 누설 방지 assert를 추가했다. marker 문자열은 기존처럼 고정하되, 실제 보안 의도인 key 미포함도 함께 검증한다.
- `kraddr-geo-ui/README.md` 검증 섹션에 WSL ext4에서는 Linux Node/npm을 사용하라는 경고를 추가했다.
...
```bash
git diff --check
TMPDIR=/tmp TMP=/tmp TEMP=/tmp .venv/bin/python -m py_compile src/kraddr/geo/cli/main.py src/kraddr/geo/loaders/shp/polygons_loader.py
