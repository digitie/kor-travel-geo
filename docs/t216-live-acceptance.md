...
등록된 group은 모두 `available` 경로로 들어갔다. 구조 검증 결과는 `warning`이며, 주요 원인은 일부 원천의 `.prj` 부재와 당시 상세 profile이 없던 optional single-file category였다. 실패는 없다. 이 warning은 이번 run-validation의 presence/integrity gate를 막지 않았고, optional category별 상세 구조 validator 강화는 T-127에서 닫았다.
