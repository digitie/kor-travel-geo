...
다음 PR은 full-load/MV/restore 완료 hook의 `ops.dataset_snapshots`/`ops.serving_releases` 자동 생성을 우선 진행한다. T-049에서 schema/API/UI 골격은 준비됐으므로, 성공 지점에서 어떤 source set과 row count가 운영에 노출됐는지를 자동 기록하는 hook을 좁은 단위로 연결한다.
