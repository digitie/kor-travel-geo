...
```bash
ktgctl uploads cleanup --dry-run
ktgctl uploads cleanup --ttl-days 30 --active-grace-minutes 360
```

정리 규칙:

- 기본 TTL은 `KTG_UPLOAD_SET_TTL_DAYS=30`이다.
- 기본 active grace는 `KTG_UPLOAD_SET_ACTIVE_GRACE_MINUTES=360`이다.
...
```bash
KTG_BACKUP_CALLBACK_MAX_ATTEMPTS=3
KTG_BACKUP_CALLBACK_BACKOFF_MS=500
KTG_BACKUP_CALLBACK_SECRET=
```

- `KTG_BACKUP_CALLBACK_MAX_ATTEMPTS`는 1~10 범위이며 기본값은 3이다.
- `KTG_BACKUP_CALLBACK_BACKOFF_MS`는 첫 retry 전 대기 시간이며 기본값은 500ms다. retry 간격은 `backoff_ms * 2^(attempt-1)`로 증가한다.
- `KTG_BACKUP_CALLBACK_SECRET`이 있으면 callback HMAC 전용 secret으로 사용한다. 비어 있으면 download token secret 또는 DSN 기반 fallback secret을 재사용한다.

전송 계약:

- callback URL은 기존과 같이 `KTG_BACKUP_CALLBACK_ALLOWED_HOSTS` allowlist host만 허용한다.
...
- HTTP header는 `x-kor-travel-geo-event`, `x-kor-travel-geo-callback-id`, `x-kor-travel-geo-timestamp`, `x-kor-travel-geo-signature: sha256=<hex>`를 포함한다.
...
- 외부 수신자가 서명을 검증해야 하는 운영에서는 `KTG_BACKUP_CALLBACK_SECRET`을 반드시 설정한다. 미설정이면 송신자는 내부 download-token secret 또는 `pg_dsn` 파생 secret으로 서명하지만, 외부 수신자는 그 값을 재현할 수 없어 검증 가능한 webhook 계약이 되지 않는다.
...
- 단독 `ktgctl refresh mv` 또는 `/admin/maintenance/refresh-mv` 경로는 `release_kind='manual_rebuild'` active release를 만든다.
...
```bash
KTG_OPS_TABLE_STATS_CAPTURE_INTERVAL_MINUTES=0
KTG_OPS_TABLE_STATS_CAPTURE_LIMIT=500
KTG_OPS_TABLE_STATS_CAPTURE_ON_STARTUP=false
```

- `KTG_OPS_TABLE_STATS_CAPTURE_INTERVAL_MINUTES=0`이면 비활성화된다. 기본값은 0으로 두어 개발 서버나 테스트 실행이 예기치 않은 DB write를 만들지 않게 한다.
- 1 이상의 값으로 설정하면 FastAPI lifespan에서 background task를 만들고, 지정한 분 간격마다 `AdminRepository.capture_table_stats_snapshots()`를 실행한다.
- `KTG_OPS_TABLE_STATS_CAPTURE_ON_STARTUP=true`이면 서버 시작 직후 한 번 capture한 뒤 주기 loop로 들어간다. 기본값은 false라 첫 capture는 첫 interval 이후 수행된다.
- `KTG_OPS_TABLE_STATS_CAPTURE_LIMIT`는 한 번에 수집할 `pg_class` object 수이며 기본값은 500, 최대 2,000이다.
...
- 수동 API/클라이언트 capture는 scheduler와 같은 advisory lock을 쓰되, lock 충돌을 빈 성공 응답으로 숨기지 않는다. 이미 다른 capture가 진행 중이면 `409 E0409`로 실패해 운영자가 "0행 수집"과 "동시 실행으로 skip"을 구분할 수 있다. scheduler 경로만 `skip_if_locked=True`로 조용히 건너뛴다.
...
- `replace_current` 확인 문구는 `RESTORE <현재 DB 이름>`이다. 예: `RESTORE kor_travel_geo`.
...
- maintenance window는 기간 gate이며 성공 시 자동 소비하지 않는다. active window 안에서 여러 복원 승인이 가능하므로 운영자는 window 종료 시점을 명시적으로 관리해야 한다. 대신 각 `replace_current` 인가 통과는 `ops.audit_events(action='maintenance_window.authorize')`에 window id, job id, target DB, confirmation hash로 기록한다.
...
T-049/T-050의 운영 메타데이터는 단위 테스트에서 SQL 문자열과 repository 동작을 확인했지만, FK·trigger·partial unique index는 PostgreSQL catalog에 실제로 올라간 뒤에만 의미가 있다. 7차에서는 `KTG_TEST_PG_DSN`이 설정된 경우에만 실행되는 선택형 통합 테스트를 추가했다. 이 DSN은 `postgis`, `pg_trgm`, `unaccent`, `pg_stat_statements` extension package를 사용할 수 있는 disposable test DB를 가리켜야 한다.

```bash
KTG_TEST_PG_DSN=postgresql+psycopg://addr:addr@localhost:15432/kor_travel_geo_t050_ops_constraints \
...
- DSN 대상 DB 이름이 `test`를 포함하거나 `kor_travel_geo_t*`, `tmp_*` 형태가 아니면 skip한다. `SCHEMA_SQL`/`INDEX_SQL`은 idempotent지만 DDL은 commit되므로 운영 DB 오지정을 방지하기 위한 guard다.
- 필수 extension package가 없는 일반 PostgreSQL DB는 schema 적용 전에 skip한다.

### 7차 검증

- DSN 미설정 경로: `1 skipped`
- 실제 Docker PostgreSQL 별도 DB `kor_travel_geo_t050_ops_constraints`: `1 passed`
