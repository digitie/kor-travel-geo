...
- AI 에이전트 작업은 `~/dev/geo-codex`, `~/dev/geo-claude`, `~/dev/geo-antigravity`처럼 에이전트별 고정 worktree에서 수행하고, 작업마다 branch만 새로 만듭니다. CodeGraph 인덱스는 각 worktree에서 최초 1회 `codegraph init -i`, 이후 `codegraph sync`로 유지합니다(ADR-034).
