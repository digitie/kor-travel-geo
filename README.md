...
> [!TIP]
> 운영 콘솔은 `/admin/load` 및 `/debug/geocode`로 바로 진입합니다. 지도 영역은 `maplibre-gl`과 VWorld 타일을 사용하며, API 요청은 Next.js Route Handler(`KRADDR_GEO_API_INTERNAL_URL`)를 통해 프록시 처리됩니다.
...
- 원천 ZIP/SHP/TXT, 외부 API 응답 캐시 덤프 파일은 본 저장소에 커밋하지 마십시오.
- 운영자는 공공데이터포털, vworld 등의 최신 약관과 API 호출 일일 한도를 직접 확인하고 관리해야 합니다.
