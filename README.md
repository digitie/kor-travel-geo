...
> [!NOTE]
> **현재 상태**: `main` 브랜치는 PostgreSQL + PostGIS 기반 재구현의 백엔드·REST·CLI와 `kraddr-geo-ui` 디버그/관리 UI를 포함합니다. T-005~T-042 구현·실데이터 검증 기록, T-049 운영 메타데이터 1차 구현, T-045 source set 기준월 선택과 대용량 업로드/적재 UX, T-046 DB 백업/복원 UI와 대구 부분 DB 실제 backup/restore 검증이 반영되어 있습니다. T-027 최종 클린 적재에서는 실제 전국 `data/juso`를 빈 Docker PostGIS DB에 처음부터 적재해 `mv_geocode_target=6,416,637`, `tl_sppn_makarea=24,204`, smoke `OK`를 확인했습니다. T-047 주요 benchmark와 T-057 region hint 1차 실측, T-044 `maplibre-vworld-js` 0.1.0 기준 문서-only 재확인, T-056 `python-kraddr-base` Address 코드 helper 독립 구현 정리, T-052 v1/v2 API 분리와 AI-friendly API reference까지 완료했습니다. 이전 SpatiaLite 기반 구현(같은 `kraddr.geo` 패키지)은 `v1` 브랜치에 보존되어 있습니다(ADR-001).
...
- AI 에이전트 작업은 `~/dev/geo-codex`, `~/dev/geo-claude`, `~/dev/geo-antigravity`처럼 에이전트별 고정 worktree에서 수행하고, 작업마다 branch만 새로 만듭니다. CodeGraph 인덱스는 각 worktree에서 최초 1회 `codegraph init -i`, 이후 `codegraph sync`와 `codegraph status`로 유지합니다(ADR-034).
- 프로젝트 루트의 `.codex/config.toml`은 CodeGraph MCP stdio 서버를 등록합니다. Codex Desktop 재시작 후 `codegraph_explore` 도구가 노출되면, `kraddr-geo-ui` 컴포넌트나 지도 wrapper 수정 전 영향도 평가에 먼저 사용합니다.
