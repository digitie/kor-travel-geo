...
// way as the VWorld key so an operator sets it once in the backend .env. A browser must
// never embed `KTG_DAGSTER_URL`: it is an internal service address and is commonly an
// unreachable loopback URL. If no browser-facing URL is configured, DagsterEmbed renders
// its configuration guidance instead. This value is NOT used for backend GraphQL.
