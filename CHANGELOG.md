...
- **Prometheus pull 계측과 관리자 UI 제품명을 정리했다.** UI의 정확한 aggregate scrape 경로
  ...
  HTTP method label은 표준 method allowlist 밖의 값을 `other`로 접고, Web Vitals name/rating/route와
  값은 유한한 allowlist·known page route·non-negative finite 검증을 거친다.
...
### Fixed
- **라이브 e2e에서 나온 경미 결함 6건을 정리했다(이슈 #515).** 로그인 페이지가 인증이 필요한
  `/api/runtime-config`·web-vitals를 호출해 매번 401 콘솔 오류를 남기던 것을 호출하지 않도록 했고,
  닫힌 모바일 드로어가 화면 밖에서 여전히 Tab 포커스를 받던 것을 `visibility`로 탭 순서에서
  제외했다(데스크톱 사이드바는 영향 없음). `/admin/tables`가 복원·hot-swap 직후 모든 테이블을
  0행으로 표시하던 것은 `n_live_tup`이 0으로 초기화되기 때문이라, 플래너가 쓰는
  `pg_class.reltuples`로 폴백한다. 제목이 없던 관리 페이지 8개에 `metadata`를 추가하고,
  `/admin/ops`가 아무 작업 전에도 가짜 `{"status":"READY"}`를 보여주던 것을 빈 상태 안내로,
  공개 API 키를 폐기해도 화면에 남아 있던 평문을 지우도록 고쳤다.
- **320/375px에서 관리 표와 원천 파일 카드가 화면 밖으로 잘려 접근할 수 없던 문제를 고쳤다(이슈 #514).**
  grid/flex 자식의 기본 `min-width: auto`와 카드 그리드의 `minmax(260px, …)` 하한 때문에 좁은 화면에서
  내용이 컨테이너를 넘었고, 전역 `overflow-x: clip`이 그것을 스크롤 대신 숨겼다. 이제 표는 전용 가로
  스크롤 영역(`role="region"`, 키보드 포커스 가능 — 정렬 헤더가 버튼이라 브라우저가 스크롤러에 자동
  포커스를 주지 않는다)을 갖고, 카드 그리드는 좁은 화면에서 가용 폭까지 수축한다.
- **로그아웃한 세션 쿠키로 관리 화면과 런타임 설정에 계속 접근되던 문제를 고쳤다(이슈 #513).**
  세션 폐기 목록이 모듈 스코프 `Map`이었는데 번들러가 `lib/auth.ts`를 레이어(라우트 핸들러 /
  페이지 SSR / Edge middleware)마다 **별도 인스턴스로** 내보내, 로그아웃 라우트가 채운 목록을
...
- T-188 T-177G smoke sample query timeout을 해소했다. 전국 `mv_geocode_target` smoke sample 선택에서 전체 후보 `ORDER BY CASE WHEN EXISTS (...)` 정렬을 제거하고, locsum-linked entrance row 1건을 `LIMIT 1`로 고르는 쿼리로 줄였다.
- T-186 T-177G post-load ops ledger 기록을 추가했다. 전국 file-driven long-run DB에서 serving MV swap 뒤 기존 `ops.dataset_snapshots`/`ops.serving_releases` hook을 호출해 live Admin UI가 active release를 읽을 수 있게 했고, T-183/T-184 착수 전 live UI e2e 20건 통과를 확인했다.
- T-185 T-177G link evidence 집계 timeout 재발을 해소했다. `collect_t177f_link_evidence()`에 호출 단위 `statement_timeout_ms` 옵션을 추가하고, 전국 long-run post-load에서만 timeout을 해제한다.
- T-182 T-177G long-run 디스크 여유 preflight를 추가했다. 전국 e2e가 7z/전자지도 materialize와 대형 DB 적재를 시작하기 전에 artifact filesystem free space를 검사하며, 기본 120GiB 요구량은 `KTG_TEST_FULL_LOAD_E2E_LONGRUN_MIN_FREE_GB`로 조정할 수 있다.
...
- T-245 복원 장애 주입 live 통합 테스트를 추가했다. `tests/integration/test_backup_restore_fault_injection.py`가 T-244 round-trip fixture로 만든 실제 백업 artifact에 archive-level sha256 flip, truncated tar, 내부 `checksums.sha256` 위조, checksum 누락을 주입하고, 각 복원 실패 뒤 job-owned target DB가 `restore_failed_target_cleanup="drop"` 정책으로 남지 않음을 검증한다. 백업 cancel은 failed artifact와 최종 archive/`.part`/work dir 삭제를 확인하고, `replace_current`는 `target_dsn` 금지·typed confirmation·maintenance window confirmation matching guard를 고정한다. 기본 CI는 `KTG_TEST_PG_DSN` 또는 `pg_dump`/`pg_restore`/`zstd` 부재 시 skip된다.
- T-238 백업 manifest↔DB↔RustFS 3자 reconcile을 추가했다. `ktgctl backup reconcile-source`가 백업 artifact 또는 `manifest.json`의 `source_match_set` per-file을 DB `ops.source_files`와 RustFS `HEAD` 결과에 대조해 `present`/`missing`/`head_error`/`etag_mismatch`/`size_mismatch`/DB 불일치를 JSON report로 낸다. RustFS 비활성 또는 legacy manifest는 `skipped=true`로 graceful 처리하고, live DB/RustFS 검증은 `KTG_TEST_RUSTFS_SOURCE_RECONCILE=1` opt-in integration test로만 실행한다.
- T-144 성능 우선 v2/API 계약 후보 검증을 추가했다. 현재 v2 계약의 `include_geometry=false`, `response_model_exclude_none=True`, geocode/search 상한 100을 성능 기준 profile로 고정하고, geometry endpoint 분리·field slim mode·detail expansion·pre-shaped response table은 T-105 후속 또는 새 구조 실험 근거가 생길 때 분리하기로 ADR-059에 기록했다. `scripts/evaluate_t144_api_contract.py`는 REST benchmark `api-report.json`의 p99/평균 응답 크기 budget을 판정한다.
- T-162 런타임 캐시/버퍼 예열 자동화를 추가했다. `loaders.runtime_warm`이 `pg_prewarm` 확장 존재 여부와 서빙 relation 존재 여부를 확인한 뒤, 선택형 `pg_prewarm`과 geocode exact/search text/reverse nearest/region radius 읽기 전용 probe를 상한 있는 쿼리로 실행한다. API lifespan에는 기본 비활성 `KTG_RUNTIME_WARM_ON_STARTUP`/`KTG_RUNTIME_WARM_INTERVAL_MINUTES` 스케줄러를 붙였고, worker 중복 실행은 `RUNTIME_WARM` advisory lock으로 막는다. `scripts/run_t162_runtime_warm.py`는 plan/execute/report/artifact 등록을, `scripts/evaluate_t162_cold_warm_ratio.py`는 재기동 직후 REST p99와 예열 후 REST p99 비율 gate를 제공한다.
...
- T-163 60분 soak resource guard를 추가했다. `scripts/run_t141_load_matrix.py` schema v2가 soak profile 실행 중 runner process RSS/CPU/`/proc/self/io` sample을 남기고, RSS 증가 budget, leak 판정식, CPU/read/write budget, error 0 기준을 `soak-guard.json`과 `matrix-report.json`에 기록한다. `--soak-guard-mode enforce`는 guard 실패 시 artifact를 쓴 뒤 exit code `2`로 종료한다.
...
### Changed
- 프로젝트 라이선스를 MIT에서 `GPL-3.0-only`로 변경하고, `LICENSE`, 패키지 메타데이터, README 고지를 갱신했다.
- **(BREAKING)** T-106 v1 geocode/reverse HTTP 응답을 VWorld envelope에 맞췄다. `/v1/address/geocode`와 `/v1/address/reverse` 정상 응답은 최상위 `response`로 감싸며, `service.name=address`, `service.operation=getCoord|getAddress`, 응답 `type=ROAD|PARCEL|BOTH` 대문자 직렬화, geocode `simple`/`refine` 생략 규칙, reverse `simple` 파라미터, VWorld식 `response.error.level/code/text` 에러 객체를 반영한다. 자체 보강 필드는 ADR-003/ADR-053에 따라 `response.x_extension`에 유지한다. OpenAPI와 UI 생성 타입을 함께 갱신했다.
...
- T-178a Claude Code 리뷰 후속으로 v2 geocode 보조 road 후보 조회를 best-effort로 바꿨다. Primary geocode OK 응답을 얻은 뒤 supplemental `GeometryRepository.road_geometries()`가 실패해도 이미 계산된 primary 후보 응답을 반환하고 warning log만 남긴다.
- PR #210~#217 사후 리뷰 후속으로 restore dry-run/restore version guard와 실패 cleanup을 보강했다. `target_dsn`이 외부 클러스터를 가리키는 경우에도 실제 target PostgreSQL/PostGIS 버전을 조회하고, target DB 접속·존재 확인 실패는 dry-run warning이 아니라 blocker가 된다. restore 실패 cleanup은 검증된 DB identifier만 quote하며, quarantine 이름은 PostgreSQL 63자 제한 안에 들어오도록 생성한다.
- Windows에서 `TL_SPPN_MAKAREA` ZIP source를 발견할 때 ZIP 내부 prefix가 `\`로 기록되어 member lookup/test가 깨질 수 있던 문제를 수정했다. ZIP member path는 `PurePosixPath`로 처리한다.
- PR #142/#145/#149 리뷰 후속으로 source registry 운영 경로를 보강했다. register 시점은 slot coverage만 검증하고 full archive member 검증은 별도 validate 경로에 남기며, `revalidate_group`은 child state를 먼저 정리한 뒤 aggregate를 재계산한다. janitor multipart abort는 실제 upload object key를 사용하고, reconcile은 RustFS object scan limit을 적용하며, 실제 등록 없는 `import_object` resolve와 RustFS client 없는 `delete_object` DB hard-delete를 차단한다.
