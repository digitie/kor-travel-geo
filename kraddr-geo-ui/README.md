...
기본 진입점은 `/debug/geocode`이며 공식 로컬 UI 포트는 `9002`이다. `KRADDR_GEO_API_INTERNAL_URL`은 서버 사이드 프록시가 사용할 백엔드 주소이고, 브라우저는 `NEXT_PUBLIC_API_BASE_URL` 기본값인 `/api/proxy`만 호출한다. 지오코딩/역지오코딩 디버그 화면은 `/v2/geocode`, `/v2/reverse` REST API를 사용하며, 관리·정규화·EXPLAIN 화면은 아직 `/v1/admin/*` 운영 API를 사용한다.
...
브라우저 e2e는 Playwright로 수행하되 Windows Node/브라우저 환경에서만 실행한다. WSL에서는 `npm run test:e2e`나 `npx playwright test`를 실행하지 않는다. UI 서버는 WSL에서 실행한다. WSL 서버에 Windows Playwright를 붙일 때는 `next dev --hostname 0.0.0.0 --port 9002` 또는 `next start --hostname 0.0.0.0 --port 9002`로 띄우고, Windows 터미널에서 WSL IP를 `PLAYWRIGHT_BASE_URL`로 지정한다. PR 완료 전 e2e는 Chrome 기준 `chromium` project와 Firefox 기준 `firefox` project를 모두 실행한다.

```bat
set PLAYWRIGHT_BASE_URL=http://<WSL_IP>:9002
...
저장소 루트의 `scripts/docker_app.sh`를 사용한다. 이 스크립트는 API/UI 이미지를 빌드하고, 컨테이너 실행 시 `.env` 또는 `kraddr-geo-ui/.env.local`에서 VWorld 키를 읽어 `KRADDR_GEO_VWORLD_API_KEY`/`NEXT_PUBLIC_VWORLD_API_KEY` 환경변수로 주입한다. 키 값은 출력하지 않는다.
...
API와 RustFS까지 함께 띄우려면 `scripts/docker_app.sh build` 뒤 `scripts/docker_app.sh up`을 사용한다. 기본 실행은 Docker bridge network와 host port mapping을 사용하므로 브라우저 진입점은 `http://127.0.0.1:9002/debug/geocode`다. 스크립트는 API `9001`, UI `9002`, RustFS S3 `9003`, RustFS console `9004` host 포트를 점유한 기존 컨테이너나 listen 프로세스를 종료한 뒤 새 컨테이너를 올린다. API 컨테이너의 RustFS endpoint 기본값은 Docker network alias `http://kraddr-geo-rustfs:9003`이다.
...
```bat
set PLAYWRIGHT_BASE_URL=http://localhost:9002
