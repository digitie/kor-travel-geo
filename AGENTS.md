...
- **테스트를 작성했으면 그 테스트가 실제로 판별하는지 증명할 것.** 수정 전 코드(또는 고의로
  되돌린 버전)에 대해 그 테스트를 돌려서 **실패하는 것**을 직접 본 뒤에야 통과를 신뢰한다.
  green 테스트 자체는 증거가 아니다 — 단언이 의도한 것과 다른 것을 보고 있을 수 있고(이름 vs
  실제 호출, 부분 문자열 vs 온전한 식별자, 주석 vs 실행되는 코드), 그 경우 오구현도 정확히
  같은 이유로 통과한다. 오구현을 몇 가지 변형으로 실제로 넣어보고 각각 다른 단언에서 실패하는지
  확인하는 쪽이, 테스트 코드를 읽고 "맞아 보인다"고 판단하는 것보다 훨씬 더 잘 잡아낸다.
- **안전장치(가드·거부·잠금)를 추가하기 전에 그것이 실제로 막을 대상과 실제로 통과시켜야 할
  대상을 먼저 조사할 것.** "이름이 이 패턴에 맞으면 위험하다/안전하다"처럼 이름이나 표면적
  패턴만으로 근사하면, 보호하려던 대상 자체가 그 패턴에 우연히 들어맞아 반대로 뚫리거나
  (예: 임시라는 뜻의 접미사가 실제로는 "보존해야 할 기준 데이터"의 명명 규칙과 겹치는 경우),
  런타임에 이름이 생성되는 정당한 대상이 걸러져 조용히 동작을 멈추는 일이 생긴다(예: 이름을
  실행 중에 만드는 하네스는 사전에 어떤 이름 규칙으로도 허용 목록에 넣을 수 없다). 두 방향 모두
  실측 없이는 알 수 없다 — 실제로 쓰이는 이름·값을 문서·이력에서 모으고, 새 가드를 넣은 뒤
  기존에 정상 동작하던 경로가 여전히 도는지 실행해서 확인한다. 가드 자체가 옳아도, 어떤 대상에
  적용했는지 확인하지 않으면 커버리지를 조용히 삭제할 수 있다 — 그리고 그 삭제는 green 테스트
  스위트 뒤에 숨어 티가 나지 않는다.
