...
> **로컬 필독 (커밋 안 됨)**: 각 워크트리 루트의 `AGENTS.local.md` — 배포·검증·머지에서 **반복되는 실수**를
> 상세 기록하고, 민감 운영 정보(prod 호스트/IP, secret 변수명·형식·위치)와 push 전 보안 감사 grep 타깃을 담는다.
> git이 추적하지 않으므로(`.gitignore` + `.git/info/exclude`) 워크트리마다 수동 복사한다. 새 사고가 나면
> 이 파일의 "사고 로그"에 추가한다. 비민감 반복 실패 패턴의 정본은 커밋된 `docs/runbooks/agent-failure-patterns.md`.
...
gitignored `AGENTS.local.md`(워크트리별 로컬 복사본) §0/§1에 있다.
...
2. **비밀 패턴 스캔** — diff에서 secret/API 키/패스워드 해시(`pbkdf2_sha256$…`)/DB DSN(`…://user:pass@…`)/
   private key/실 IP(prod 호스트)를 grep으로 점검한다(구체 패턴은 `AGENTS.local.md` §0).
3. **로컬 비밀 파일 확인** — `.env`·`.env.local`·`app.env`·`*.local.*`·`settings.local.json`·`AGENTS.local.md`가
...
배포·검증·머지에서 반복되는 실수와 회피책은 gitignored `AGENTS.local.md`(상세·민감)와 커밋된
