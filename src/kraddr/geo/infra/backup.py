...
DEFAULT_BACKUP_DISPLAY_PREFIX = "kraddr_geo_backup"
ROW_COUNT_OBJECTS = (
    "tl_juso_text",
    "tl_juso_parcel_link",
    "tl_locsum_entrc",
...
def resolve_backup_destination(destination_dir: str | None, settings: Settings) -> Path:
    ...
    if not roots:
        msg = "KRADDR_GEO_BACKUP_ALLOWED_DIRS must contain at least one directory"
        raise InvalidInputError(msg)
    if destination_dir:
...
"""
    ...
    return {
        ...
        "git_commit": os.environ.get("KRADDR_GEO_GIT_COMMIT", "unknown"),
        "database": {
            "name": db["name"],
