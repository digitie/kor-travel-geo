...
CASE_PREPARE_SQL: dict[str, str] = {
    "C4": """
DROP TABLE IF EXISTS _ktg_dq_c4_distances;
CREATE TEMP TABLE _ktg_dq_c4_distances ON COMMIT DROP AS
WITH distances AS (
  SELECT j.bd_mgt_sn,
         e.ent_man_no,
         j.rncode_full,
...
CREATE INDEX _ktg_dq_c4_distances_dist_idx
  ON _ktg_dq_c4_distances (dist_m DESC);
ANALYZE _ktg_dq_c4_distances;
""",
    "C6": """
DROP TABLE IF EXISTS _ktg_dq_c6_violations;
CREATE TEMP TABLE _ktg_dq_c6_violations ON COMMIT DROP AS
WITH base AS (
  SELECT j.bd_mgt_sn,
         j.zip_no,
         e.ent_man_no,
...
ANALYZE _ktg_dq_c6_violations;
""",
    "C7": """
DROP TABLE IF EXISTS _ktg_dq_c7_violations;
CREATE TEMP TABLE _ktg_dq_c7_violations ON COMMIT DROP AS
WITH base AS (
  SELECT j.bd_mgt_sn,
